    ...
    <header-navigator :currentComponentName.sync="currentComponentName" @changeMenuStatus="isShowDropMenu=$event">
      <div class="header-menu-container" v-if="isShowDropMenu && currentComponentName !== 'AppDownload'">
        <div class="header_menu_body">
          ...
          <QrCode v-if="currentComponentName !=='HomeMenu'" :showRightEN="true" :topTitle="returnWaterData[currentComponentName].returnWater? $t('返水') : ''" :returnWater="returnWaterData[currentComponentName].returnWater" :currentComponentName="currentComponentName" :enFlag="returnWaterData[currentComponentName].en_flag">
            <div>{{$t('扫一扫')}}</div>
            <div>{{$t('下载全站APP')}}</div>
          </QrCode>
        </div>
...
export default {
  ...
  components: {
    ...
    FishMenu: () => import(/* webpackChunkName: "FishMenu" */ './component/FishMenu'),
    BingoMenu: () => import(/* webpackChunkName: "BingoMenu" */ './component/BingoMenu.vue'),
    DouJiMenu: () => import(/* webpackChunkName: "DouJiMenu" */ './component/DouJiMenu'),
  ...
  computed: {
    ...
    ...mapState('game', ['sport', 'douji', 'electronic', 'catchfish']),
	},
  methods: {
    ...mapMutations('game', ['set_esport', 'set_catchfish', 'set_bingo', 'set_douji', 'set_sport', 'set_real', 'set_poker', 'set_lottery', 'set_electronic']),
    ...mapMutations('normal', ['set_render_slot_game', 'set_render_catchfishGame', 'set_render_pokerGame', 'set_render_lotteryGame', 'set_render_realGame']),
    //- 渲染静态游戏列表到stroe
    initGameList () {
        ...
        if (item.catId > 0) {
          ...
          //- 获取电子游戏列表 及捕鱼数据
            ...
            if (eRes.data.code) return
              ...
              // bingo
              if (this.isPhp) {
                const bingoArr = this.baseMenuData.bingoMenu.list;
                bingoArr.forEach(item => {
                  const target = this.electronic.find(itemAll => itemAll.depotCode === item.depotCode);
                  if (target) {
                    item.id = target.id;
                  }
                })
                const newBingoArr = bingoArr.filter(item => item.id);
                this.set_bingo(newBingoArr);
              }
							return;
						}
