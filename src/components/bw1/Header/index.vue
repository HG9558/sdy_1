  ...
  <div>
    <div :class="`header-container ${!isShowDropMenu?'drop-menu-hidden':'drop-menu-show'}`" ref="header">
      <header-navigator :currentComponentName.sync="currentComponentName" @changeMenuStatus="isShowDropMenu=$event">
        <div class="header-menu-container" v-if="isShowDropMenu && currentComponentName">
          <div class="header_menu_body">
            ...
            <QrCode v-if="currentComponentName !=='HomeMenu'" :showRightEN="true" :topTitle="returnWaterData[currentComponentName].img_src?'返水最高可达':''" :returnWater="returnWaterData[currentComponentName].returnWater" :currentComponentName="currentComponentName" :enFlag="returnWaterData[currentComponentName].en_flag">
              <div>扫一扫</div>
              <div>下载全站APP</div>
            </QrCode>
          </div>
