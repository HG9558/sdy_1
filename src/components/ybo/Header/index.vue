<template>
  <div :class="`header-container ${!isShowDropMenu?'drop-menu-hidden':'drop-menu-show'}`" ref="header">
    <header-navigator :currentComponentName.sync="currentComponentName" @changeMenuStatus="isShowDropMenu=$event">
      <div class="header-menu-container" v-if="isShowDropMenu">
        <div class="header_menu_body">
          ...
          <QrCode v-if="currentComponentName !=='HomeMenu'" :showRightEN="true" :topTitle="returnWaterData[currentComponentName].img_src?'返水最高可达':''" :returnWater="returnWaterData[currentComponentName].img_src" :currentComponentName="currentComponentName" :enFlag="returnWaterData[currentComponentName].en_flag">
            <div>扫一扫</div>
            <div>下载全站APP</div>
          </QrCode>
        </div>
...
export default {
  ...
  methods: {
    ...
    watchScroll () {
      ...
      if (scrollTop > 300) {
        return this.$refs.header.style.backgroundColor = 'rgb(255,255,255, 0.9)'
      }
