<template>
  <Layout :length="gameState.fish.length" :num="4">
    <div
      ...
      v-for="(item, index) in gameState.fish"
      :key="index"
      ...
      <div class="fish-item">
        <div class="fish-context">
          <img
            class="bg-img"
            :src="useImgPath(`base/header_fish_${item.depotCode.toLowerCase()}.png`)"
            alt=""
            @error="$event => useErrorPath('base/header_fish_default.png', $event)"
          />
...
const goElectronicPage = (item: string) => {
  gameState.set_fish_depotCode(item);
