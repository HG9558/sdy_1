<template>
  <div :class="['esport_component_container', site]">
    <div
      @click="$emit('enterGame', item)"
      v-for="(item, index) in gameState.esport"
      :key="index"
      class="electronic-item">
      <div class="esport-content">
        <img
          class="bg-img"
          :src="useImgPath(`base/header_electronic_${item.depotCode.toLowerCase()}.png`)"
          alt=""
          @error="$event => useErrorPath('base/header_electronic_default.png', $event)"
        />
        <div class="desc-txt">
...
import { useImgPath, useErrorPath, useSite } from '@/hook/usePath';
