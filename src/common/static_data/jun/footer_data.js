...
let linkUrlListMgm = [
  { value: lang('博彩责任'), url: "/game_help?index=0" },
  { value: lang('隐私保护'), url: "/game_help?index=1" },
  { value: lang('规则与条款'), url: "/game_help?index=2" },
  { value: lang('联系我们'), url: "/contact_us" },
  { value: lang('加入合营'), url: "/partner" },
]
...
export const linkUrlList = (function () {
  if (VUE_APP_APP_SIGN === 'lua') {
    return linkUrlListLua
  }

  if (VUE_APP_APP_SIGN === 'mgm') {
    return  linkUrlListMgm
  }

  if (VUE_APP_APP_SIGN === 'php') {
    return  linkUrlListDefault.filter( v=> v.url !== '/partner')
  }
