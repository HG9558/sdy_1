...
export function getTime (val) { // 获取今天、昨天、本周、上周、本月、上月的开始及结束时间
  let Nowdate = new Date()
  let getDay = Nowdate.getDay() || 7
  let yesterday = new Date(Nowdate - 86400000) // 昨天
  let tomorrow = new Date( Nowdate.getTime() + 86400000) // 明天
  let afterTomorrow = new Date( Nowdate.getTime() + 86400000 * 2) // 后天
  let threeday = new Date(Nowdate - 86400000 * 2) // 三天内
  let sevenDaysAgo = new Date(Nowdate - 86400000 * 6) // 7天前
  let thirtyDaysAgo = new Date(Nowdate - 86400000 * 29) // 30天前
  let fortyFiveDaysAgo = new Date(Nowdate - 86400000 * 44) // 45天前
  let WeekFirstDay = new Date(Nowdate - (getDay - 1) * 86400000) // 本周第一天
  let WeekLastDay = new Date((WeekFirstDay / 1000 + 6 * 86400) * 1000) // 本周最后一天
  let preWeekFirstDay = new Date((WeekFirstDay / 1000 - 7 * 86400) * 1000) // 上周第一天
  let preWeekLastDay = new Date((WeekFirstDay / 1000 - 86400) * 1000) // 上周最后一天
  ...
  let MonthLastDay = new Date(MonthNextFirstDay - 86400000) // 本月最后一天
  let preMonthLastDay = new Date(MonthFirstDay - 86400000) // 上个月最后一天
  let M1, M2, d1, d2
  switch (val) {
    case '今天':
...
export function dataTime (unixTime, type = "Y-M-D H:i:s") { // 转换时间
...
export function handleBeforeUpload (file) { // 上传代码判断
