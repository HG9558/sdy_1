<template>
  <div class="recommend-wrapper-table">
    <table class="table-container">
      ...
      <tbody>
        <tr v-for="(item, index) in tableData" :key="index">
          <td @click="cellClick(renderItem, item[renderItem])" :class="{ sbuStyle: renderItem === 'subLoginName' }" v-for="(renderItem, index) in labelList" :key="index">
            {{ item[renderItem] || "-" }}
          </td>
        </tr>
    ...
    <el-pagination v-if="showPagination" class="common-page" align="right" @current-change="handleCurrentChange" :current-page.sync="pageNo" layout="total, prev, pager, next" :total="total" :page-size="5">
    </el-pagination>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async _getFriendRebateFriendsRewardSummary () {
      const res = await this.$http.getFriendRebateFriendsRewardSummary();
