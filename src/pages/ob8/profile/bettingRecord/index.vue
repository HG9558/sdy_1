  ...
  <div>
    <div class="top-div">
      <div class="head-container">
        <div class="head-left">
          <div class="head-left-top">记录查询 (当前系统支持查询最近45日的投注记录)</div>
            ...
            <template slot-scope="props">
              <el-form label-position="left" inline class="demo-table-expand">
                <el-form-item label="订单号" style="width: 235px;">
                  ...
                  <span style="margin-left: 10px; color: #61B2F9; cursor: pointer;float: right;" v-clipboard:copy="props.row.id" v-clipboard:success="firstCopySuccess" v-clipboard:error="firstCopyError">复制</span>
                </el-form-item>

                <el-form-item style="text-align: center;" label="状态">
                  <span style="color: #61B2F9;">{{ props.row.status }}</span>
                </el-form-item>

                <el-form-item :label="props.row.odds ? '赔率' : ''" >
                  <span v-if="props.row.odds" style="color: #61B2F9;">{{ props.row.odds }}</span>
                </el-form-item>

                <el-form-item v-if="props.row.payoutTime" style="text-align: center;" label="派彩时间">
                  <span style="color: #61B2F9;">{{ props.row.payoutTime }}</span>
                </el-form-item>

                <el-form-item v-if="props.row.payoutTime" style="text-align: center;" label="玩法类型">
                  <span style="color: #61B2F9;">{{ props.row.playType }}</span>
                </el-form-item>

                <el-form-item v-if="props.row.leagueName" style="text-align: center;" label="联赛名称">
                  <span style="color: #61B2F9;">{{ props.row.leagueName }}</span>
                </el-form-item>

                <el-form-item v-if="props.row.betScore" style="text-align: center;" label="下注比分">
                  <span style="color: #61B2F9;">{{ props.row.betScore }}</span>
                </el-form-item>

                <el-form-item v-if="props.row.team" style="text-align: center;" label="比赛队伍">
                  <span style="color: #61B2F9;">{{ props.row.team }}</span>
                </el-form-item>

                <!-- <el-form-item style="color: #61B2F9;" v-show="isShowPreSettle(props.row)">
                  <div>
                    <span @click="setOrderPreSettle(props.row)" style="cursor: pointer; border: 1px solid #66b1ff; border-radius: 4px; padding: 2px 5px; "> 提前结算</span>
                  </div>
                </el-form-item> -->
              </el-form>
            </template>
