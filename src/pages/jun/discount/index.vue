<template>
  <div class="discount-wrapper" id="box">
    <img :src="siteImgMap.defaultBanner" v-if="!bannerlist.length" width="100%" alt="">
    <el-carousel v-else arrow="never" class="discount-banner">
      <el-carousel-item v-for="(_, i) in bannerlist" :key="i"><img @click="goDetailPage(_)" :src="_.picPcPath" />
      </el-carousel-item>
    </el-carousel>
    <SplitLine />
    <div class="discount-content">
      <el-tabs v-if="Object.keys(discountData).length" class="discount-tabs" v-model="activeName" tab-position="left" @tab-click="changeType">
        <el-tab-pane v-for="(item,index) in Object.values(discountData)" :name="item.catName" :key="item.id">
          ...
          <div class="icon-catName" slot="label" v-if="item.disable === '1'">
            <img class="icon" :src="getIcon(item, index)">
      ...
      <div class="no-data" v-show="shwoNodata">
        ...
        <div class="no-data-title">{{$t('暂无数据')}}</div>
      </div>
...
<!--    <RedEnvelopeEntrance />-->
    ...
    <global-dialog :title="$t('优惠详情')" :showDialog.sync="showDialog" className="discount-dialog" :width="'800px'"
      :hasSelfFooter="true">
...
export default {
  ...
  created () {
    ...
    if (Object.keys(this.discountData).length) {
			!this.discountData[this.discountType] && this.set_discount_type(Object.keys(this.discountData)[0]);
			this._getDataList();
    } else {
  ...
  computed: {
    ...
    ...mapGetters('global', ['isZhLanguage']),
		activeName: {
      get () {
				return (this.discountData[this.discountType] && this.discountType) || Object.keys(this.discountData)[0]
			},
      set (val) { }
    },
  },
  methods: {
    ...
    ...mapMutations('normal', ['set_discount_type', 'set_link_data', 'set_scroll_top']),
		getIcon(item, index) {
			try {
				return require(`./img/icon_${index+1}${this.activeName===item.catName?`_active`:''}.png`)
			} catch (e) {
				return require(`./img/icon_10${this.activeName===item.catName?`_active`:''}.png`)
			}
		},
    changeType (data) {
