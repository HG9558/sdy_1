      ...
      <template v-if="drawConfig.prizesList && drawConfig.prizesList.length">
        <div class="zhuanpan-wrapper" @click="startCallBack()">
          ...
          />
				...
				<!-- i18n组件插值 -->
				<i18n class="luck-draw-content_number" path="剩余x次抽奖机会" tag="div">
					<span place="remainingTimes" class="luck-draw-content_number_dot">{{ activeLotteryInfo.remainingTimes || 0 }}</span>
				</i18n>
      </template>
    </div>
    <div class="bottom-wraper">
			<div class="activity-title">
				<img src="./img/titleImgLeft.png" alt="">
				<span>{{ $t('活动规则') }}</span>
				<img src="./img/titleImgRight.png" alt="">
			</div>
      <div
    ...
    async startCallBack() {
      if (!this.running) {
        if (!this.activeLotteryInfo.remainingTimes > 0) {
          this.messageTip({ message: this.$t('抽奖机会已用完，请先获取抽奖机会') });
          return
        }
        this.running = true
        this.$refs.LuckyWheel.play()
        await this.accountLottery()
        this.endCallBack()
      }
    },
    endCallBack() {
      ...
      if (prize.prizeInfo) {
        ...
        if (prize.prizeInfo.prizeType !== 1) {
          ...
          this.messageTip({ message: this.$tt('恭喜您获得：xxx, 请联系客服进行领取', { title: prize.title }) });
        } else {
