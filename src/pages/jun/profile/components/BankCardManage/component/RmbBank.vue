<template>
  <el-form class="bog add-bank-card-form" :model="rmbBankForm" :label-width="isZhLanguage ? '130px' : '185px'" ref="rmbBankForm" size="small" :rules="rmbBankFormDataRule">
    ...
    <el-form-item prop="realName" :label="$t('真实姓名：')">
      ...
      <el-form-item v-if="isSelectBank && !isPhp" label="* BSB：" prop="address">
    ...
    <template v-else>
			...
			<el-form-item v-if="isMgm" :label="$t('开户银行区域：')">
				<el-col :span="11">
					<el-input :placeholder="$t('州属')" v-model="rmbBankForm.province" :maxlength="60"></el-input>
				</el-col>
				<el-col class="line" :span="2" style="text-align: center">-</el-col>
				<el-col :span="11">
					<el-input :placeholder="$t('区/区域')" v-model="rmbBankForm.city" :maxlength="60"></el-input>
				</el-col>
			</el-form-item>
...
export default {
  ...
  data () {
		const isBw1Aud = VUE_APP_APP_SIGN === 'bw1' && VUE_APP_APP_BANK_ARE_CODE === 'AUD'
    return {
			isBw1Aud,
      rmbBankForm: {
        address: "",
      ...
      rmbBankFormDataRule: {
        ...
        address: isBw1Aud ? [{ validator: this.validateBSB, trigger: "blur" }] : [],
        bankName: [{ required: true, trigger: 'change', message: isBw1Aud ? this.$t('请选择提款方式') : this.$t('请选择开户银行') }],
			},
		}
  },
  methods: {
