      ...
      <div class="tab-container">
        ...
        <div class="tab-main">
          <div
            ...
            :key="index"
            :class="{'money-type':true, on: componentName=== item.componentName }"
            @click="(componentName = item.componentName) && (formName = item.formName)"
          >
            {{ item.text }}
          </div>
        </div>
...
export default {
  ...
  watch: {
    ...
    tabActive: {
      immediate: true,
      handler (val) {
        const value = this.tabList.find( v => v.name === val ) || this.tabList[0]
        if (value) {
      ...
      const arr = [
        {
          ...
          text: this.$t('银行卡'),
          isShow: this.cnyWithdrawEnable,
        },
        {
          name: 'crypto',
          componentName: 'CryBank',
          formName: 'cyBankForm',
          text: 'USDT',
          isShow: this.crFlag,
        },
        {
          name: 'alipay',
          componentName: 'AliBank',
          formName: 'alipayBankForm',
          text: this.$t('支付宝'),
          isShow: this.alipayEnable,
        },
        {
          name: 'elec',
          componentName: 'ElecBank',
          formName: 'elecBankForm',
          text: this.$t('电子钱包'),
          isShow: this.isPhp && this.eMoneyPayWithdrawEnable,
        },
      ]

      return arr.filter( v => v.isShow)
    },
    isPhp() {
