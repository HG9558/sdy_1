<template>
  <div class="cr-pay-container rm">
    <div v-if="!showLoading && !noData">
      <div v-if="false" class="pay-type-container">
...
export default {
  ...
  data () {
    return {
      ...
      money: null,
      renderList: [
        { label: this.$t('电子钱包'), children: [], imageUrl: 'new_bank' },
      ],
      noticeTextData,
      feeForm: {
        fee: null, //充值数值
  ...
  methods: {
    ...
    async _initRmb () {
      ...
      const resList = await Promise.allSettled([this.$http.getPzpayPictureUrl(), this.$http.getFastBankList({ areaCode: VUE_APP_APP_BANK_ARE_CODE || 'CN' })])
      resList[0].status === 'fulfilled' && this.getPzpayPictureUrl(resList[0].value);
      resList[1].status === 'fulfilled' && this.getFastBankList(resList[1].value);
    ...
    getPzpayPictureUrl (response) {
      ...
      //- 电子钱包
      res.emoneyPayList && res.emoneyPayList.length && this.renderList[0].children.push(...res.emoneyPayList);
    ...
    async getFastBankList (response) {
      ...
      this.renderList[0].children.unshift(...elecList);
        ...
        if (this.curPayChannel.alipayFlg === 1) {
          try {
            ...
            this.openDialog({
              ...
              dialogTitle: `${this.bankDialogData.bankName} ${this.$t('支付')}`,
