  ...
  <div class="cr-pay-container">
      ...
      <div class="select-pay-form">
        ...
        <div class="select-box">
          ...
          <div class="select-form-container">
            ...
            <el-button class="handle-btn bog" @click="_cyPayDeposit" :loading="buttonLoading">{{$t('立即存款')}}</el-button>
    ...
    <global-dialog v-if="crChannelList.length" class="cr-dialog" :title="`${crChannelList[selectIndex].crName}(${crChannelList[selectIndex].category})`" :showDialog.sync="showDialog" :hasSelfFooter="false" :cancelText="$t('已完成充值')" :confirmText="$t('充值问题需要协助')" @confirmHandle="confirmHandle">
      <div class="cr-pay-dialog">
        ...
        <el-button class="bog cr-button" type="primary" v-clipboard:copy="crPayInfo.walletAddress" v-clipboard:error="onCopyError" v-clipboard:success="onCopySuccess">{{$t('点击复制钱包地址')}}</el-button>
  ...
  methods: {
    ...
    async _initCrypto () {
      ...
      try {
        ...
        if (crListRes.data.crList && crListRes.data.crList.length) {
