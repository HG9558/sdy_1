  ...
  <div class="withdrawal-container">
    ...
    <div class="withdrawal-wrapper">
      ...
      <template v-else>
        ...
        <div v-if="isBw1Aud" class="tip">
          {{ $t('由于澳洲银行首次转账会有Pending问题，如果首次提款没立刻到账请耐心等待24小时，谢谢。') }}
        </div>
        <!-- <div v-if="isPhp" class="tip">
          {{ $t('由于菲律宾银行首次转账会有Pending问题，如果首次提款没立刻到账请耐心等待24小时，谢谢。') }}
        </div> -->
...
export default {
  ...
  methods: {
    ...
    initTabActive() {
      if (this.cnyWithdrawEnable) {
        this.tabActive = 'rmb'
      } else if (this.crFlag) {
        this.tabActive = 'crypto'
      } else if (this.alipayEnable) {
        this.tabActive = 'alipay'
      } else if (this.isPhp && this.eMoneyPayWithdrawEnable) {
        this.tabActive = 'elec'
      }
    },
