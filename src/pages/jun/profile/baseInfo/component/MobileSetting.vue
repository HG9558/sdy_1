    ...
    <div class="profile-dialog">
      <el-form :model="mobileForm" class="bog mobile-form" ref="mobileForm" size="small" :rules="mobileRules">
        <el-form-item :rules="activeMobileAreaCfg.rules" prop="mobile">
          ...
          <el-input class="mobile-phone" clearable :maxlength="mMaxlen" :placeholder="$t('请输入手机号码')" v-model="mobileForm.mobile"></el-input>
        </el-form-item>

        <el-form-item prop="code">
          <el-input class="smcode-input" v-model="mobileForm.code" :maxlength="6" clearable :placeholder="$t('请输入短信验证码')" type="number">
      ...
      <el-button class="self-footer" type="primary" @click="_bingMobile" :loading="loading">{{$t('确定')}}</el-button>
    </div>
  </div>
</template>
...
export default {
  ...
  data () {
      ...
      mobileRules: {
				...
				captchareg: [
          { required: true, message: this.$t('请输入图形验证码'), trigger: "blur" },
        ],
      },
    }
  },
  methods: {
    _bingMobile () {
      this.$refs.mobileForm.validate(async (valid) => {
        ...
        this.messageTip({ message: this.$t('手机绑定成功'), type: 'success' })
        //- 重新获取用户信息
        this._getUserInfo();
      });
    },
      ...
      if (res.data.code === 0) {
		...
		validatePhoneCode (rule, value, callback) {
			if (!value) {
				...
				callback(new Error(this.$t('请输入正确的短信验证码')));
				this.isDisabeld = true;
			} else {
    ...
    mMaxlen() {
      return this.mobileAreaCode === '63' ? 10 : this.activeMobileAreaCfg.maxlength
    }
	},
  watch: {
    mobileForm: {
