<template>
  <div class="feedback-wrap">
    <div class="feedback-type">{{$t('问题类型')}}</div>
    <el-form class="bog feedback-form" ref="form" :model="form">
      <el-form-item prop="selectValue">
        <el-select popper-class="bog" v-model="selectValue" :placeholder="$t('请选择问题类型')">
    ...
    <el-form class="feedback-form" ref="form" :model="form">
      <el-form-item prop="desc">
        <el-input ref="desc" minlength="20" maxlength="200" class="bog resize-none" type="textarea" v-model="form.desc" :placeholder="$t('请输入内容介于20-200字的问题描述')"></el-input>
      </el-form-item>
      ...
      <div class="upload-inner">
        <el-upload :on-remove="()=>imgData=null" ref="upload" class="upload-demo" drag :limit="1" action="" :show-file-list="true" :auto-upload="false" :on-change="changeHandle" list-type="picture" accept=".jpg,.png,.jpeg">
          <div class="upload-container" v-if="!imgData">
            ...
            <div class="el-upload__text" style="color:  $c-1884;">{{$t('请将文件拖到此处，或')}}<em style="color: #606266">{{$t('点击上传')}}</em></div>
            <div style="color:$c-1884;">{{$t('（文件格式为PNG或JPG，且大小不超过15MB，目前支持上传一张）')}}</div>
          </div>
        </el-upload>
      </div>
    </div>

    <el-button :loading="loading" @click="btnClick()" type="button" class="btnCommit">{{$t('提交反馈')}}</el-button>
