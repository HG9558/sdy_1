  ...
  <div class="vipinfo">
    <div class="vipinfo-top middle" v-if="isShowVipList">
      <el-carousel type="card" :initial-index="nowInitialIndex" arrow="never" indicator-position="none" :autoplay="false" height="211px">
        <el-carousel-item :class="`v-${index}`" v-for="(item, index) in activityLevelList.slice(1, 8)" :key="index" :style="{
    ...
    init () {
      ...
      this.$http.getVipInfo().then((res) => {
        if (res.data.code === 0) {
          ...
          const levelData = this.vipData.activityLevelCatDtos.filter(v => v.accountLevelId < 9);
