      ...
      <div class="pay-type-container">
        <div :class="{'py-type-list':true,on:index === payListIndex}" @click="changePayListIndex(index)" v-for="(item,index) in  renderList" :key="index">
          <div class="circle-outer">
            <p v-if="item.isShow && item.isHot" class="tag"><i>热门</i></p>
            <p v-if="item.isShow && !item.isHot && item.isRecommend" class="tag"><i>推荐</i></p>
        ...
        <div :class="{'select-pay-item rm-pay-item':true,on:index === payChannelIndex}" @click="changePayChannelIndex(index)" v-for="(item,index) in renderList[payListIndex].children" :key="index">
          ...
          <div class="pay-name">
            ...
            <span class="number">
              ...
              :`${item.minAmout}-${item.maxAmout}` }} MYR
            </span>
          </div>
      ...
      <div class="select-pay-form">
        ...
        <div class="select-box rm-box">
          <div class="select-card-container">
            <div :class="{'select-item rm-select-item':true,on:recommendMoneyIndex === index}" v-for="(item,index) in recommendList" :key="index" @click="changeRecommendMoneyIndex(index,item)">{{item.label}}MYR</div>
          </div>
...
export default {
  ...
  data () {
    return {
      ...
      money: null,
      renderList: [
        { label: "网银转账", children: [], imageUrl: 'new_bank' },
        { label: "支付宝", children: [], imageUrl: 'new_alipay' },
        { label: "微信", children: [], imageUrl: 'new_wechat' },
        { label: "电子钱包转账", children: [], imageUrl: 'new_unionpay' },
  ...
  methods: {
    ...
    async _initRmb () {
      ...
      const resList = await Promise.allSettled([this.$http.getPzpayPictureUrl(), this.$http.getBankList(), this.$http.getFastBankList({ areaCode: 'MY' })])
    ...
    getPzpayPictureUrl (response) {
      ...
      if (payTypeList && payTypeList.length) {
        this.renderList = this.renderList.map(item => {
          const pickRes = payTypeList.filter((itemPayType) => (itemPayType.name === item.label))
      ...
      if (this.curPayChannel.payType === 'qr') {
        ...
        if (this.curPayChannel.alipayFlg === 1) {
          try {
            ...
            //- 大额支付专用
            this.openDialog({ componentName: "LargePaymentDialog", dialogTitle: '银行卡支付', cancelText: "已完成充值", confirmText: "充值问题需要协助" })
          } catch (error) {
            console.log(error)
            this.buttonLoading = false;
            this.$refs["feeForm"].resetFields();
