<template>
  <div class="coupons">
        ...
        <div class="coupons-wrap-border">
          <div class="coupons-wrap-first">
            ...
            <div class="scoend-div"
              ...
              v-if="!isMixingRule(item) ">{{ item.amountMax }}MYR</div>
          </div>
          <div class="coupons-wrap-second" v-if="!isMixingRule(item)">
            <span v-if="!isSpecialActive(item)">
              领取要求：{{ tmplCode(item).label }}≥{{ tmplCode(item).value }}MYR
            </span>
            <span v-if="isSpecialActive(item) && tmplCode(item).value">
              {{ tmplCode(item).label }}:{{ tmplCode(item).value }}MYR
            </span>
          </div>
            ...
            <template v-if="isMixingRule(item)">
              <div v-for="(_item, _i) in mixingDetail" :key="_i" class="mixing-detailitem">
                <div class="mixing-topbox">
                  <span class="white-color">领取要求：{{_item.activityName}}——{{ tmplCode(_item).label }}≥{{
                  tmplCode(_item).value }}MYR</span>
                  <span>
                    <span class="scoend-div mixing-title">{{_item.activityName}}</span>
                    <span class="scoend-div mixing-title">{{_item.amountMax}}MYR</span>
    ...
    <global-dialog title="领取提示" :showDialog.sync="isShowDialog" class="unbing-wrap" :hasSelfFooter="true">
      ...
      <div class="d-btn-div">
        <div @click="isShowDialog = false">取消</div>
  ...
  methods: {
    ...
    getApply () {
      ...
      this.$http.getActApply(data).then((res) => {
        if (res.data.code === 0) {
          this.messageTip({ type: 'success', message: "成功领取优惠" });
          this.getData();
        } else {
