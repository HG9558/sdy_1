  ...
  <div class="recommend-wrapper">
    ...
    >
          ...
          <div class="line-content">
            ...
            <table class="table-wrapper">
              ...
              <tr class="table-content">
                <td>{{ rebateFirstChargeDto.minCharge }}-{{ rebateFirstChargeDto.maxCharge }}MYR</td>
          ...
          <table class="table-wrapper">
            <tr class="table-header">
              ...
              <th v-for="(item, index) in rebateChargeDtoList" :key="index">
                {{ item.award }}MYR礼金({{ item.multiple }}倍流水)
              </th>
            </tr>
            <tr class="table-content">
              <td rowspan="4">条件</td>
      ...
      <div class="footer mt-20">
        <!-- <button class="btn rule-btn" @click="proportionShow = false">取消</button>
        <button class="btn confirm-btn" @click="proportionShow = false">确定</button> -->
      </div>
    </global-dialog>
    ...
    <global-dialog class="rule-wrapper" :showDialog.sync="promoteShow" title="推广" width="500px" :hasSelfFooter="true">
      <div class="content promote-wrapper">
        ...
        <div class="wrap-text">或<span class="copy-url">复制链接到浏览器打开</span></div>
        <div class="friend-url"><span>好友注册链接</span></div>
        <div class="url">{{ agentUrl }}</div>
      </div>
      <div class="footer promote-footer">
        <button class="btn rule-btn" v-clipboard:copy="agentUrl" v-clipboard:success="copySuccess">复制推广链接</button>
        <button class="btn confirm-btn" @click="saveQrCode">保存推广页面</button>
      </div>
    </global-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    async _getRemmendRule() {
      try {
        ...
        this.rebateVipDtoList = this.rebateVipDtoList.filter(item => item.level <= 7)
