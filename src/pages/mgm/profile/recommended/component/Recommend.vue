    ...
    <div class="recommend-wrapper">
      <div class="amount-wrapper">
        <div class="item">
          <div class="amount-inner">
            <div class="title">昨日收益（MYR）</div>
            <div class="amount">{{ rebateSummary.ydActualReward }}</div>
            <div class="cumulativeAmount">累计金额：{{ rebateSummary.sumActualReward }}MYR</div>
          </div>
        </div>
        <div class="item">
          <div class="amount-inner">
            <div class="title">昨日首存款返利（MYR）</div>
            <div class="amount">{{ rebateSummary.ydFirstChargeReward }}</div>
            <div class="cumulativeAmount">累计金额：{{ rebateSummary.sumFirstChargeReward }}MYR</div>
          </div>
        </div>
        <div class="item">
          <div class="amount-inner">
            <div class="title">昨日有效投注额返利（MYR）</div>
            <div class="amount">{{ rebateSummary.ydValidBetReward }}</div>
            <div class="cumulativeAmount">累计金额：{{ rebateSummary.sumValidBetReward }}MYR</div>
          </div>
        </div>
        <div class="btns">
          <div class="top">
            <button class="btn top-btn" @click="$emit('openDialog', 'rule')">规则说明</button>
            <button class="btn top-btn" @click="$emit('openDialog', 'proportion')">比例</button>
          </div>
          <div class="bottom">
            <button class="btn bottom-btn" @click="$emit('update:componentName', 'MyFriend')">我的好友</button>
            <button class="btn bottom-btn" @click="$emit('openDialog', 'promote')">立即推广</button>
          </div>
        </div>
      </div>
      <div class="daily-earnings">
        <div class="earnings-title">
          <div>收益日报</div>
          <div class="more" @click="$emit('update:componentName', 'GetMore')">更多>></div>
        </div>
        <!-- 表格组件 -->
...
.content-title_inner {
  border-bottom: 1.2px solid #deccba;
}
.en-title {
  width: 456px;
  right: 200px;
