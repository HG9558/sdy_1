<template>
  <div v-show="visible" class="red-packet-wrapper">
    <div class="clockTxt" v-if="openCountDown || endCountDown">{{$t('倒计时')}}</div>
    <div class="clockTxt" v-if="openCountDown">{{ openCountDown.h_in_d }}:{{ openCountDown.m }}:{{ openCountDown.s }}{{$t('开始')}}</div>
    <div class="clockTxt" v-else-if="endCountDown">{{ endCountDown.h_in_d }}:{{ endCountDown.m }}:{{ endCountDown.s }}{{$t('结束')}}</div>
