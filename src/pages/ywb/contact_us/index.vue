  ...
  <div class="contact-us-box">
    <div class="contact-us-content">
      <div class="contact-us-content-inner">
        <div class="contact-us-content-inner_title">{{$t('联系我们')}}</div>
      ...
      <div class="contact-us-input-box">
        <el-form :model="from" :rules="validateRule" ref="from">
          <el-form-item prop="name">
            <el-input class="user-input" v-model="from.name" :maxlength="20" clearable :placeholder="$t('名字')"></el-input>
          </el-form-item>
          <el-form-item prop="number">
            <el-input class="user-input" v-model="from.number" :maxlength="11" clearable :placeholder="$t('联络号码')"></el-input>
          </el-form-item>
          <el-form-item prop="email">
            <el-input class="user-input" v-model="from.email" clearable :placeholder="$t('邮件地址')"></el-input>
          </el-form-item>
          <el-form-item prop="description">
            <el-input ref="description" :maxlength="200" class="resize-none" type="textarea" v-model="from.description" :placeholder="$t('说明')"></el-input>
          </el-form-item>
          <el-form-item class="btn-class">
            <el-button :loading="isLoading" class="btn-pubilc" @click="dataFormSubmitHandle('from')">{{$t('提交')}}</el-button>
          </el-form-item>
        </el-form>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    validateRule () {
      ...
      return {
        name: [{ required: true, message: this.$t("名字不能为空"), trigger: "blur" }],
        number: [
        ...
        description: [
          { required: true, message: this.$t("说明不能为空"), trigger: "blur" },
        ],
      };
    },
  },
  methods: {
    ...
    dataFormSubmitHandle (data) {
      this.$refs[data].validate(async (valid) => {
        ...
        formData.append(
          "textContent",
          `${this.$t('【名字】')}${this.from.name}${this.$t('【联络号码】')}${this.from.number}${this.$t('【邮件地址】')}${this.from.email}${this.$t('【说明】')}${this.from.description}`
        );
        try {
