  ...
  <div class="add-bank-card-dialog">
    <div class="add-bank-card-form-wrapper">
      ...
      <div class="tab-container">
       ...
       <div class="tab-main">
					<!-- goa站点仅支持USDT -->
          <div v-if="!isGoa" :class="{'money-type':true, on: componentName=== 'RmbBank'}" @click="(componentName='RmbBank')&&(formName='rmbBankForm')">{{$t('银行卡')}}</div>
...
<!--          <div :class="{'money-type':true, on: componentName=== 'OtherPay' }" @click="(componentName = 'OtherPay')&&(formName = 'otherPayForm')">其他钱包</div>-->
        </div>
      </div>
      <!--  澳币添加  虚拟币添加 支付包添加 -->
      <keep-alive>
    ...
    <el-button class="yszjl add-bank-card-btn" :loading="bindBtnLoading" @click="_handleAddBankcard(formName)" width="250" height="36">{{$t('确认')}}</el-button>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    tabActive: {
      immediate: true,
      handler (val) {
        this.componentName = val === 'rmb' ? 'RmbBank' : val === 'alipay' ? 'AliBank' : val === 'crypto' ? 'CryBank' : 'OtherPay'
        this.formName = val === 'rmb' ? 'rmbBankForm' : val === 'alipay' ? 'alipayBankForm' :  val === 'crypto' ? 'cyBankForm' : 'otherPayForm'
      }
    },
  },
  data () {
            ...
            this.requestFun(form).then((res) => {
              if (res.data.code === 0) {
                ...
                const messageTipData = {
                  ...
                  cyBankForm: {
                    text: this.$t('USDT钱包添加成功'),
                    http: '_getCryptoList',
                  },
                  alipayBankForm: {
                    text: this.$t('支付宝账户添加成功'),
                    http: '_getAlipayList'
                  }
                }
