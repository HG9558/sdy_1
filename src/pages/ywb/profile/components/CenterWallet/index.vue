  ...
  <div :class="{ 'center-wallet-container': true, 'has-box-shadow': $route.name !== 'wallet'}" >
    <div class="common-container">
      ...
      <div class="totoal-money-container">
        ...
        <div class="bottom-handle">
          <div class="return-btn" @click="_recycling">{{$t('一键回收')}}</div>
          <div class="line"></div>
          <div class="lock-container">
            <img class="icon_lock" src="./img/wallet_icon_lock.png" alt="" />
            <div class="text">
              <div>{{$t('锁定钱包')}}</div>
              <div class="money">{{$tt('currencySymbol')}}{{ lockingMoney }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="game-wallet-container" v-if="$route.name === 'wallet'">
      <div class="game-z-z">
        ...
        <div class="money" v-show="!isAutoPay">{{$t('场馆钱包')}}</div>
      </div>
      ...
      <div v-else>
        <div class="game-sport" v-for="(item, index) in platList" :key="index">
          <div class="game-11" :class="item.availableWh === 2 ? 'maintenance' : ''">
            <el-tooltip class="game-sport-one-tooltip" :content="item.depotName" placement="top">
              <div class="game-sport-one">{{ item.depotName }}</div>
            </el-tooltip>
            <div class="game-sport-two">
              <i v-if="item.availableWh === 2" class="el-icon-warning warning"></i>
              {{ item.availableWh === 2 ? $t('维护中') : item.moneyBlance }}
            </div>
          </div>
          <div class="game-transfers" v-show="item.availableWh !== 2">
            <div class="transfer-on" @click="toggleIn(item.depotName, item.id, centerBalanceMoney)">{{$t('转入')}}</div>
            <div class="transfer-out" @click="toggleOut(item.depotName, item.id, item.moneyBlance)">{{$t('转出')}}</div>
          </div>
        </div>
      </div>
      <!-- 弹窗 -->
  ...
  methods: {
    ...
    setManuPay () {
      ...
      this.setDialog({
        ...
        dialogContent: this.$t('关闭自动免转后,您将需要手动操作各个游戏钱包的金额,确定关闭自动免转?'),
    ...
    toggleIn (depotName, id, money) {
      this.curDepotId = id
      this.setDialog({
        dialogTitle: `${this.$t('中心钱包至')}${depotName}`,
        dialogContent: money,
        componentName: 'TransferPlatformDialog',
        checkText: this.$t('中心钱包余额不足,请先充值'),
        type: 'in',
        depotId: id
      })
    },
    toggleOut (depotName, id, money) {
      this.curDepotId = id
      this.setDialog({
        dialogTitle: `${depotName}${this.$t('至中心钱包')}`,
        dialogContent: money,
        componentName: 'TransferPlatformDialog',
        checkText: this.$t('金额超过场馆金额'),
        type: 'out',
        depotId: id
      })
    },
