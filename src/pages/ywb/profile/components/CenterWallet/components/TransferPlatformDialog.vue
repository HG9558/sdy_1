<template>
  <div class="money-dialog">
    <el-form :model="moneyData" class="yszjl" ref="moneyData" size="small" :rules="moneyRule">
      <el-form-item prop="money">
        <div>{{$t('转账金额:')}}</div>
        <el-input class="money-input" :placeholder="$t('请输入金额')" v-model="moneyData.money">
        </el-input>
        <span class="max-money" @click="setMaxMoney">{{$t('最大金额')}}</span>
      </el-form-item>
    </el-form>
    <el-button class="self-footer" type="primary" @click="sumbit" :loading="loading">{{$t('立即转账')}}</el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    validateMoney: (rule, value, callback) => {
      ...
      if (!value) {
        callback(new Error(this.$t('请输入金额')));
      } else if (!reg.test(value)) {
        callback(new Error(this.$t('请输入最多两位小数的数值')));
