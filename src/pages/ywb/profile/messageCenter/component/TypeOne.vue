...
<!--        </div>-->
<!--      </el-upload>-->
<!--      <el-input class="yszjl send-input" v-model="sendMessage" clearable placeholder="输入内容点击发送">-->
<!--      </el-input>-->
...
export default {
  ...
  methods: {
    ...
    async sendClick () {
      this.isGetMore = false;
      if (!this.sendMessage) return this.messageTip({ message: this.$t('发送内容不能为空') });
      let formData = new FormData();
      formData.append("textContent", this.sendMessage);
      const res = await this.$http.getMessageCenterSend(formData);
      if (res.data.code) return this.messageTip({ message: res.data.msg });
      this.sendMessage = '';
      this.$emit('triggerSelectClick', { val: this.$t('收件箱'), type: 1, icon: 'message', currentType: 'TypeOne', noLoading: true });
    }
  },
  deactivated () {
    this.isGetMore = false;
