  ...
  <div class="cr-pay-container rm">
    <div v-if="!showLoading && !noData">
      ...
      <div class="user-name-container yszjl el-form" v-if="curPayChannel.paymentType == 17 && curPayChannel.depositName">
        <span>{{$t('存款姓名：')}}</span>
        <el-input v-model="accountName" class="name-input" type="text" :placeholder="$t('请输入存款人姓名')" />
        <div class="notice">
      ...
      <div class="select-pay-form">
       <div class="tip">
        ...
        <div class="select-box rm-box">
          ...
          <div v-if="curPayChannel.amountType !==1" class="notice">{{$t('由于银行风控原因 请不要输入整数 更快捷充值成功')}}</div>
          ...
          <div class="tip-text">
            {{renderList[payListIndex].label ===$t("本地银行")?noticeTextData[renderList[payListIndex].label] : noticeTextData[renderList[payListIndex].label]+$t('请按照提交后页面实际显示金额充值，否则无法自动到账！')}}
          </div>
        </div>
      </div>
