<template>
  <div class="cr-pay-container rm">
    <div v-if="!showLoading && !noData">
      ...
      <div class="user-name-container yszjl el-form" v-if="curPayChannel.paymentType == 13&&curPayChannel.depositName">
        <span>{{$t('存款姓名：')}}</span>
      ...
      <div class="select-pay-form">
        ...
        <div class="select-box rm-box">
          ...
          <div class="select-form-container">
            ...
            <el-button :class="{'handle-btn':true,'yszjl':true , 'rm-handle-btn':curPayChannel.amountType ===1}" @click="_rmPayDeposit" :loading="buttonLoading">{{$t('立即存款')}}</el-button>
            <div class="handle-btn yszjl" @click="$router.push('/help_center')">{{$t('存款教程')}}</div>
          </div>
      ...
      if (this.curPayChannel.payType === 'qr') {
        ...
        if (this.curPayChannel.alipayFlg === 1) {
          try {
            ...
            //- 大额支付专用
            this.openDialog({ componentName: "LargePaymentDialog", dialogTitle: `${this.bankDialogData.bankName}${this.$t('大额支付专用')}`, cancelText: this.$t('已完成充值'), confirmText: this.$t('充值问题需要协助'), hasSelfFooter: true })
          } catch (error) {
            console.log(error)
            this.buttonLoading = false;
            this.$refs["feeForm"].resetFields();
          ...
          try {
            ...
            this.openConfirmDialog({
              content: [this.$t('请在新页面完成充值支付'), this.$t('充值支付完成前。')],
              cancelButtonText: this.$t('请勿关闭此窗口'),
              confirmButtonText: this.$t('已完成充值'),
              callback: () => { window.open(this.serUrl) },
              cancelFn: () => this._getCenterMoney(),
              customClass: 'deposit-tips-dialog'
            })
          } catch (error) {
            console.log(error)
            this.buttonLoading = false;
