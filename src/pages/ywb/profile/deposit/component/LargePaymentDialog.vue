<template>
  <div class="dialog-wrapper">
    <div class="dialog-content" v-for="(item,index) in renderList" :key="index">
      ...
      <div class="copy" v-clipboard:copy="String(item.value)" v-clipboard:error="onCopyError" v-clipboard:success="onCopySuccess">
        <img class="copy-img" src="../../img/deposit/copy.png" alt="">
        <span class="copy-text"> {{$t('点击复制')}} </span>
      </div>
    </div>
    <div v-if="certType > 0" class="upload-box">
      <div>*{{ $t('上传凭证：为了您的存款能快速到账，请先上传存款凭证') }}</div>
    ...
    <div v-if="curRmbChannel && curRmbChannel.label === $t('支付宝')">
      <div>{{$t('操作步骤:')}}</div>
      <div class="setup">
    ...
    <div v-else>
      <div>{{$t('操作步骤:')}}</div>
      <div class="setup">
        <div>{{$t('1.登录您自己设备上的PAYID;')}}</div>
        <div>{{$t('2.进入转账页，将该信息复制到收款方;')}}</div>
        <div>{{$t('3.入款成功')}}<span class="service">{{$t('联系客服')}}</span>{{$t('查询进度。')}}</div>
...
export default {
  ...
  computed: {
    ...
    renderList () {
      ...
      const data = [{ realName: this.$t('收款姓名：') }, { bankAccount: this.$t('银行卡号：') }, { depositAmount: this.$t('充值金额：') }, { depositPostscript: [this.$t('附&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;言：'), this.$t('(必须正确填写)')] }];
