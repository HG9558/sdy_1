    ...
    <div class="fee-form">
      <el-form class="yszjl" :model="feeForm" :rules="feeFormRules" ref="feeForm">
        <el-form-item prop="fee" class="fee">
          <el-input class="money-form" :clearable="true" :placeholder="`${this.$t('最低')} ${minAmount}${this.$tt('currencyValue')}，${this.$t('最高')} ${maxAmount}${this.$tt('currencyValue')}`"
      ...
      this.$refs["feeForm"].validate((valid) => {
        if (valid) {
          if (
            ...
            let msg = this.tabActive === 'otherpay' ? this.$t('请先添加EBPAY') : this.tabActive === 'rmb' ? this.$t('请先添加银行卡！') : this.tabActive === 'crypto' ? this.$t('请先添加虚拟货币钱包！') : this.$t('请先添加支付宝！')
