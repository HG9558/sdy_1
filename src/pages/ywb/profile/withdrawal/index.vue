...
<!--      </div>-->
      ...
      <div :class="{'tab-list':true,'on':tabActive==='crypto'}" v-if="crFlag" @click="tabActive='crypto'">
        ...
        <span class="text">{{$t('加密货币')}}</span>
			...
			<!-- goa站点仅支持USDT -->
      <div :class="{'tab-list':true,'on':tabActive==='rmb'}" @click="tabActive='rmb'" v-if="cnyWithdrawEnable && !isGoa">
...
<!--      </div>-->
    ...
    <div class="withdrawal-wrapper">
      ...
      <div class="warning-box" v-if="!userInfo.mobile" @click="$router.push('/profile')">
        ...
        <div class="tip">{{$t('为了您的资金安全，请您先操作绑定手机号！')}}</div>
      </div>
