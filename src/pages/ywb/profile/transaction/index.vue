  ...
  <div>
    <div class="top-div">
      ...
      <div class="filter-content">
        <div class="date-div">
          <el-date-picker class="date-picker" v-model="value1" type="datetimerange" :range-separator="$t('至')" :start-placeholder="$t('开始日期')" :end-placeholder="$t('结束日期')">
          </el-date-picker>
          <button @click="queryBtn" class="inquire">{{$t('查询')}}</button>
        </div>
        <div class="date-box">
...
export default {
  ...
  data () {
    return {
      ...
      componentsNameList: ['deposit', 'withdrawal', 'reward', 'discount', 'rebate', 'LastRebate'],
      paginationTotal: 0,
      timeList: getTime("今天"),
      totalData: {},
      value1: [
