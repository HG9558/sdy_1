<template>
  <div class="data-box">
    <div class="provider-box">
      ...
      <div class="bind-box">
        <div class="bind-item">
          ...
          <div class="bind-btn" @click="openDialog({componentName:'NickNameSetting',dialogTitle: $t('修改昵称'),className:'nick'})">
            {{$t('修改')}}
          </div>
        </div>
        ...
        <div class="bind-item">
          ...
          <template v-else>
            ...
            <div class="bind-btn" @click="openDialog({componentName:'MobileSetting',dialogTitle: $t('绑定手机号'),className:'mobile'})">
              {{$t('绑定')}}
            </div>
          </template>
        </div>
        <div class="bind-item">
          ...
          <template v-else>
            <div class="center">{{$t('未绑定')}}</div>
            <div class="bind-btn" @click="openDialog({componentName:'EmailSetting',dialogTitle: $t('绑定电子邮箱'),className:'email'})">
              {{$t('绑定')}}
            </div>
          </template>
        </div>
    ...
    <!-- 弹窗 -->
    <global-dialog width="610px" :showDialog.sync="showDialog"  :class="className" :title="dialogTitle" :hasSelfFooter="true">
...
export default {
  ...
  methods: {
    openDialog ({ componentName, dialogTitle, confirmText, className }) {
