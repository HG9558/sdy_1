    ...
    <div class="allTeam">
        ...
        <div class="teamList" v-if="JSON.stringify(gameList) !== '{}' ">
            <div class="list-group"  v-if="gameList.going.length">
                <div class="group-date"> {{curDate.replaceAll('/', $t('月'))+$t('日')}} {{curDate.week}} </div>
                    ...
                    <div class="group-item" v-for="item in gameList.going" :key="item.id">
                        <div class="group-item-inner">
                            ...
                            <div class="group-center">
                                ...
                                <div > {{item.grouping}} {{$t('组')}} </div>
                                <div class="group-score"> {{item.awayScore}} : {{item.homeScore}} </div>
                                <button :class="['group-status', item.matchState === '0' ? 'notStart' : 'going']"> {{ item.matchState === '0' ? $t('未开始') : item.realTime }} </button>
                            </div>
                        ...
                        <button class="btn-handle" @click="enterGame"> {{ item.matchState === '0' ? $t('提前下手') : $t('参与竞猜') }} </button>
                    </div>
                </div>
            </div>
            <div class="list-group"  v-if="gameList.end.length">
                <div class="group-date"> {{$t('比赛结束')}} </div>
                <div class="group-content">
                    <div class="group-item" v-for="item in gameList.end" :key="item.id">
                        <div class="group-item-inner">
                            ...
                            <div class="group-center">
                                ...
                                <div > {{item.grouping}} {{$t('组')}} </div>
                                <div class="group-score"> {{item.awayScore}} : {{item.homeScore}} </div>
                                <button class="group-status end"> {{$t('已结束')}} </button>$t()
                            </div>
                            <div class="group-right">
                                <img :src="item.awayLogo" alt="awayLogo">
                                <p> {{item.awayName}} </p>
        ...
        <div v-else class="no-data">
            ...
            <p>{{$t('当天暂无比赛')}}</p>
            <button class="btn-more" @click="enterGame">{{$t('更多比赛')}}</button>
        </div>
...
export default {
    ...
    methods: {
        ...
        // 格式化 星期日  return 星期*
        dayFormat(day) {
            const arr = [ this.$t('星期日'), this.$t('星期一'), this.$t('星期二'), this.$t('星期三'), this.$t('星期四'), this.$t('星期五'), this.$t('星期六') ]
