  ...
  methods: {
    ...
    async _getMatchList (freshenOnlineNum) {
      ...
      this.list = res.data.data.map((i) => {
        i.playUrls = sortSource(i.playUrls, i.matchType)
        return i
      })
    ...
    async changeMatch (match) { //- 获取点击时选取直播频道内容 （热门赛事和直播频道里面的每个直播选项会触发）
      ...
      if (!isCustomize) {
        ...
        let resData = []
        data.vids.map(item => {
          item.playUrls = sortSource(item.playUrls, item.matchType)
          resData.push({
            ...item,
            ...item.playUrls[0]
          })
        })
        data.playUrls = data.playUrls ? sortSource(data.playUrls, data.matchType) : resData
