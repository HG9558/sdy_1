<template>
  <div class="coupons">
      ...
      <div class="coupons-wrap" v-for="(item, index) in dataList" :key="index" :class="{
        ...
        <div class="coupons-wrap-border">
          <div class="coupons-wrap-first">
            ...
            <div class="scoend-div" :class="{'received-color':item.buttonShow === 3 || item.buttonShow === null, 't0': isSpecialActive(item) && !tmplCode(item).value}" v-if="!isMixingRule(item) && !isHideMoney(item)">{{ item.amountMax }}元</div>
          </div>
          <div class="coupons-wrap-second">
          ...
          <div class="coupons-wrap-three">
            ...
            <button @click="openClick(index)" class="first-btn" :class="{ 'mixing-detail': isMixingRule(item) }">
              ...
              <i class="el-icon-arrow-down" style="color: #3D4957" v-show="selectDiv === null || selectDiv !== index"></i>
              <i class="el-icon-arrow-up" style="color: #3D4957" v-show="selectDiv === index"></i>
            </button>
          ...
          <div class="coupons-wrap-last" :class="{ 'is-show': selectDiv === index }">
            <template v-if="isMixingRule(item)">
              <div v-for="(_item, _i) in mixingDetail" :key="_i" class="mixing-detailitem">
                <div class="mixing-topbox">
                  <span class="scoend-div mixing-title">{{_item.activityName}} {{_item.amountMax}}元</span>
                  ...
                  <span>                    
                    <button @click="openMixingClick(_i)" class="first-btn">
                      ...
                      <i class="el-icon-arrow-down" style="color:#3D4957" v-show="selectMixingDiv === null || selectMixingDiv !== _i"></i>
                      <i class="el-icon-arrow-up" style="color:#3D4957" v-show="selectMixingDiv === _i"></i>
                    </button>
                ...
                <div class="coupons-wrap-last detail-content" :class="{ 'is-show': selectMixingDiv === _i }"> 
                  ...
                  <div class="last-last">
                    使用范围：
                    <span v-for="(_categories, _index) in getCouponUseRange({ ..._item, depotCatDtoList: item.depotCatDtoList })" :key="_index">
            ...
            <div class="status-two" v-else>
              <div class="status-two-inner">
                <div class="first-last" v-if="!isHideMoney(item)">
                  取款流水倍数：{{
                ...
                <div class="last-last">
                  使用范围：
                  <span v-for="(categories, index) in getCouponUseRange(item)" :key="index">
    ...
    <global-dialog title="领取提示" :showDialog.sync="isShowDialog" class="unbing-wrap" :hasSelfFooter="true">
      ...
      <div class="d-div" v-if="!isHideMoney(item)">
        取款流水倍数：{{
  ...
  methods: {
    ...
    selectClick (item) {
     ...
     isSpecialActive (item) {
      ...
      // 首存活动 AQ0000001
      // 存送活动 AQ0000003
      const codes = ['AQ0000001', 'AQ0000003', 'AQ0000012']
    ...
    getRuleList (data) {
      ...
      // 存送活动 AQ0000003
      try {
        if (tmplCode === 'AQ0000012') {
          result = ruleJson.bettingGiftRuleDtos.map(item => ({
            ...item,
            amountMin: item.validBetMin,
          }))
        } else {
