<template>
  <el-form class="bog add-bank-card-form" :model="rmbBankForm" label-width="130px" ref="rmbBankForm" size="small" :rules="rmbBankFormDataRule">
    ...
    <el-form-item label="真实姓名：">
      <el-input v-if="bankDifferentName" v-model="rmbBankForm.realName" placeholder="请输入真实姓名"></el-input>
      <el-input v-else :disabled="!!isHaveReavName" v-model="rmbBankForm.realName" placeholder="请输入真实姓名"></el-input>
    </el-form-item>

    <el-form-item label="银行账号：" prop="cardNo">
      <el-input type="number" placeholder="请输入银行账号" @blur="getBankName" :maxlength="19" v-model="rmbBankForm.cardNo"></el-input>
    </el-form-item>
...
export default {
  ...
  methods: {
    //- 查询银行名称
    getBankName () {
      if(!this.rmbBankForm.cardNo) return
