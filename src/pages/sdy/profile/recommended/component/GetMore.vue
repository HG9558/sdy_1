      ...
      <div class="search-container">
        <div class="search-left">
          <div class="search-left_top">
            <span class="date-text">首存时间</span>
          ...
          <el-date-picker :editable="false" :format="currentSelect === 'month' ? 'yyyy-MM' : 'yyyy/MM/dd'" class="search-left_bottom" v-model="searchDate" type="date" placeholder="选择日期" popper-class="black-bg">
          </el-date-picker>
        </div>
        <div class="search-btn-group">
          <div class="btn search-btn" @click="_searchRecord">查询</div>
          <div class="btn reset-btn" @click="resetSetting">重置</div>
        </div>
      </div>
    </div>
    <!-- 公共表格组件 -->
    <common-table :tableData="tableData" :tableLabel="tableLabel" :pageNo.sync="pageNo" :total="total" :loading="loading"></common-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    async _searchRecord () {
      if (!this.searchDate) return this.messageTip({ message: '请先选择查询时间' });
      this.tableList = [];
      this.currentSelect === "date"
        ? this._getFriendRebateRewardReportForDay()
        : this._getFriendRebateRewardReportForMonth();
    ...
    async _getFriendRebateRewardReportForDay (type) {
      ...
      const res = await this.$http.getFriendRebateRewardReportForDay({
        ...
        endTime: type ? dataTime(new Date(), "Y-M-D") : dataTime(this.searchDate || startTime, "Y-M-D"),
      });
      this.loading = false;
