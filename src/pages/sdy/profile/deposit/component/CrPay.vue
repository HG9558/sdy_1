<template>
  <div class="cr-pay-container">
    <div v-if="!showLoading && !noData && crChannelList.length">
      <div class="tip">选择充值通道 
        <span class="usdt-txt" v-if="stationSetting.usdtBuyUrl" @click="hdclick"> <img src="../images/icon-shopcar.svg" alt=""> 购买USDT币</span>
        <router-link to="/usdtTeach" class="usdt-txt" v-if="stationSetting.usdtBuyUrl" >USDT购买教程</router-link>
      </div>
      <div class="select-pay-container">
        <div :class="['select-pay-item', {'active': selectIndex === index}]" v-for="(item,index) in crChannelList" :key="index" @click="changeRechargeChannel(item,index)">
      ...
      <div class="select-pay-form">
        ...
        <div class="select-box">
          ...
          <div class="select-form-container">
            <el-input v-model.number="money" class="bog input-money" type="number" :placeholder="feePlaceholder"></el-input>
            <el-button class="handle-btn bog btn-deposit" @click="_cyPayDeposit" :loading="buttonLoading">立即存款</el-button>
            <el-button :class="{'handle-btn bog':true,'remindActive' : isRemindIng}" @click="_onRemind">{{remindText}}</el-button>
            <el-button class="handle-btn bog" @click="$router.push('/help_center')">存款教程</el-button>
          </div>

          <div class="select-info">
            <div class="info-item tip-text">请使用您信任的第三方交易所钱包进行转账，本平台不收取任何手续费，最终充值金额以实际到账金额为准！
            </div>
            <div class="info-item exchange-num">当前参考汇率： 1 USDT ≈ {{exchangeRate}} CNY</div>
            <div class="info-item get-money">预计到账金额：{{userGetMoney}} 元</div>
          </div>
        </div>
      </div>

      <div class="pay-card-list-container">
        <div class="card-title">可任意选择支持USDT支付的交易所及钱包，例如：</div>
    ...
    <global-dialog v-if="crChannelList.length" width="475px" class="cr-dialog" :title="`${crChannelList[selectIndex].crName}(${crChannelList[selectIndex].category})`" :showDialog.sync="showDialog" :hasSelfFooter="false" cancelText="已完成充值" confirmText="充值问题需要协助" @confirmHandle="confirmHandle">
      <div class="cr-pay-dialog">
        ...
        <div>
          <div class="cr-line">
            <span class="cr-title">预计到账金额: </span>
            <span class="cr-value">{{crPayInfo.depositAmountCNY}} CNY</span>
          </div>
          <div class="cr-line">
            <span class="cr-title">支付金额：</span>
            <span class="cr-value">{{crPayInfo.depositAmount}} USDT</span>
          </div>
          <div class="cr-line">
            <span class="cr-title">钱包地址：</span>
            <span class="cr-value">{{crPayInfo.walletAddress}}</span>
            <img class="cr-copy" src="../../img/deposit/copy.png" alt="">
            <span class="cr-button" type="primary" v-clipboard:copy="crPayInfo.walletAddress" v-clipboard:error="onCopyError" v-clipboard:success="onCopySuccess">复制</span>
          </div>
        </div>

        <div class="cr-title tip">请使用您的加密货币钱包或交易所平台进行支付</div>
      </div>
    </global-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onCopyError () {
      this.messageTip({ message: "复制失败！", type: "error" });
    },
    onCopySuccess () {
    ...
    async _cyPayDeposit () {
      ...
      if (!this.money) {
        this.messageTip({ message: '存款金额不能为空' })
        return;
      }
