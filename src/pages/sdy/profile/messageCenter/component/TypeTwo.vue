<template>
  <div class="info-msg-wrap">
    <div class="clear-div" v-if="unReadCount">
      <div class="count-notice">{{ unReadCount }}条未读</div>
      <div class="clear-btn" @click="clearBtnClick">清除未读</div>
    </div>
    ...
    <!-- 弹窗部分内容 -->
    <global-dialog title="消息通知" class="message-dialog" :showDialog.sync="showDialog" :hasSelfFooter="true">
      ...
      <el-button class="bog unbing-btn-div" @click="_delMessage()">删除消息</el-button>
    </global-dialog>
...
export default {
  ...
  methods: {
    ...
    async _clearUnRead () {
      ...
      this.set_un_read_message_count(secondRes.data.count);
      this.$emit('triggerSelectClick', { type: 2, icon: 'inform', currentType: 'TypeTwo' })
    },
    async _delMessage () {
      try {
        const res = await this.$http.deleteNotice({ id: this.id });
        if (res.data.code) return this.messageTip({ message: "删除失败 接口异常", type: "error" });
        this.messageTip({ message: "删除成功" });
        this.$emit('triggerSelectClick', { type: 2, icon: 'inform', currentType: 'TypeTwo' })
