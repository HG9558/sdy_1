<template>
  <div class="discount-wrapper" id="box">
    <img src="./img/default_banner.png" v-if="!bannerlist.length" alt="" width="100%">
    ...
    <global-dialog title="优惠详情" :showDialog.sync="showDialog" className="discount-dialog" :width="'800px'"
      :hasSelfFooter="true">
...
export default {
  ...
  created () {
    ...
    if (Object.keys(this.discountData).length) {
      !this.discountData[this.discountType] && this.set_discount_type(this.textMap[process.env.VUE_APP_APP_CODE]);
      this._getDataList();
    } else {
  ...
  computed: {
    ...
    activeName: {
      get () {
        return (this.discountData[this.discountType] && this.discountType) || this.textMap[process.env.VUE_APP_APP_CODE] || '全部优惠'
      },
      set (val) { }
    }
  },
  methods: {
    ...
    async _getDiscountTypeList () {
      ...
      res.data.page.forEach(item => {
        (item.catName === '全部' || item.catName === '全部活动' || item.catName === '全部优惠') && (item.catName = this.textMap[process.env.VUE_APP_APP_CODE]);
