      ...
      <div class="select-pay-form">
        ...
        <div class="select-box rm-box">
          ...
          <div v-if="curPayChannel.amountType !==1" class="notice">注意：转账时请以转账页面的实际金额进行转账</div>
             ...
             <span v-show="renderList[payListIndex].imageUrl === 'new_unionpay'">
          <span>
            <p>温馨提示：①转账人姓名必须与平台实名一致；②支付金额必须与提交金额一致；③仅支持 银行卡转卡</p>
            <p>请勿使用 支付宝 和 微信 以及 云闪付 转卡否则将无法上分或影响上分速度，</p>
            <p>特别注意 每次存款都需要使用存款通道获取的最新的收款信息进行转账</p>
            <p>收款信息会不定期进行更换 转错将无法上分 敬请谅解</p>
            <p>如遇所选存款通道拥塞 请更换金额发起 以便快速充值</p>
          </span>
             </span>
