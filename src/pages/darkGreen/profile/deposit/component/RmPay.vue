      ...
      if (this.curPayChannel.payType === 'qr') {
        ...
        if (this.curPayChannel.alipayFlg === 1) {
          try {
            ...
            //- 大额支付专用
            this.openDialog({ componentName: "LargePaymentDialog", dialogTitle: `${this.bankDialogData.bankName} ${this.$t('银行卡支付')}`, cancelText: this.$t('已完成充值'), confirmText: this.$t('充值问题需要协助'), hasSelfFooter: true })
          } catch (error) {
            console.log(error)
            this.buttonLoading = false;
            this.$refs["feeForm"].resetFields();
