          ...
          <div class="head-left-bottom">
						<div class="plat-select">
							...
							<el-form class="bog betting-form" ref="form" :model="form">
								<el-form-item>
									<el-select popper-class="bog" @change="getTableList" v-model="selectValue" :placeholder="$t('请选择需要查询的平台')">
						...
						<div class="head-date-div">
							<el-date-picker class="date-picker clear-icon" v-model="timer" type="datetimerange" :start-placeholder="$t('开始日期')" :end-placeholder="$t('结束日期')" :picker-options="pickerOptions" @change="dateChange" :clearable="false">
							</el-date-picker>
        ...
        <el-table v-show="tableData.length !== 0" class="betting-table" :data="tableData" style="width: 100%" max-height="500"  @row-click="rowClick" ref="multipleTable">
          <el-table-column type="expand">
            <template slot-scope="props">
              <div v-if="!props.row.odds || props.row.odds.indexOf(',') === -1">
                <el-form label-position="left" inline class="demo-table-expand">
                  <el-form-item :label="$t('订单号')" style="white-space: nowrap;">
                    <span class="order-number">{{ props.row.id || "请联系客服" }}</span>
                    <span style="margin-left: 10px; cursor: pointer;float: right;" v-clipboard:copy="props.row.id" v-clipboard:success="firstCopySuccess" v-clipboard:error="firstCopyError">{{$t('复制')}}</span>
                  </el-form-item>
                  ...
                  <el-form-item v-if="props.row.playType" style="text-align: center;" :label="$t('玩法类型')">
              ...
              <div v-else>
                ...
                <template v-if="props.row.odds">
                  <el-form label-position="left" inline class="demo-table-expand" v-for="(item, index) in props.row.odds.split(',')" :key="'chuanguan-'+index" style="border-top: 1px solid rgba(255, 255, 255, 0.3)">
                    <el-form-item v-if="props.row.odds && props.row.odds.split(',')[index]" :label="props.row.odds ? $t('赔率') : ''" style="">
                      <span>{{ props.row.odds.split(',')[index] }}</span>
                    </el-form-item>
          ...
          <el-table-column prop="payout" :label="$t('输/赢')">
            <template slot-scope="scope">
							<span v-if="scope.row.status !== '已结算' && scope.row.status.trim() !== 'Settled' && scope.row.status.trim() !== 'Định cư'">-</span>
              <span v-else :style="{
