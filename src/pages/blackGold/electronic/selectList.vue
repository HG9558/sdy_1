    ...
    <div class="list-wrapper select-list-wrapper">
      <div class="game-type">
        <div class="game-classify">
          <div class="title-box">
            <img class="go-back" @click="$router.push('/electronic')" src="../electronic/img/goback.png" alt="" />
            <img class="logo" :src="require(`./img/logo_${renderData.depotCode}.png`)" :alt="$t('图片加载失败')" />
            <div class="title">
              <div>{{renderData.title}}</div>
              <div v-if="isZhLanguage" class="en-title">{{renderData.enTxt}}</div>
            </div>
          </div>
        ...
        <div class="game-list select-list" v-if="renderData.renderList.length">
          <div :key="index" class="game-item-wrap select-list-wrap" v-for="(dataItem, index) in renderData.renderList" @click="$emit('enterGame', { ...dataItem, depotCode: renderData.depotCode })">
...
export default {
  name: 'selectList',
  created () {
    !this.selectElectronicDepotCode && this.$router.push('/electronic')
  },
  data () {
    return { pageNo: 1 }
  },
  computed: {
    ...
    ...mapState({
      selectElectronicDepotCode: state => state.game.selectElectronicDepotCode,
  ...
  methods: {
    ...
    async getGameList (params) {
      const res = await this.$http.getEleDataList(params);
