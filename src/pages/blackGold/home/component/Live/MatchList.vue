...
<!--      <div class="arrow-left" :class="{'disabled': tabIndex < 1}" @click="tabIndex -= 1"> </div>-->
<!--      <div class="tab_list_p">-->
<!--        <div class="tab_list" ref="tab_list">-->
<!--          <div class="tab-item" v-for="(tab, index) in tabs" :key="tab.catId" :class="{ active: index === tabIndex }"-->
<!--            @click="tabIndex = index">-->
<!--            {{ tab.catName }}-->
<!--          </div>-->
<!--        </div>-->
<!--      </div>-->
<!--      <div class="arrow-right" :class="{'disabled': tabIndex + 1 >= 5}" @click="tabIndex += 1"> </div>-->
    </div>
<!--    <div class="giftTab" v-if="tabIndex === 4">-->
<!--      <div class="tab-item" :class="{ 'active': weekGiftLog }" @click="weekGiftLog = true">周贡榜</div>-->
<!--      <div class="tab-item" :class="{ 'active': !weekGiftLog }" @click="weekGiftLog = false">总榜</div>-->
<!--    </div>-->
    <!-- 赛事列表 -->
    <el-scrollbar wrap-class="list-wrapper" ref="scrollBar">
      <div class="match-list" v-if="tabIndex !== 4" :class="{ 'no-data': isEmpty(endObj) }">
        <div class="match-item" :class="{
          ...
          <div @click="handleChangeLive(matchItem)"
            :class="[ 'match-inner', {'playing': matchItem.matchId === matchId}]" v-for="(matchItem, index) in match"
            :key="index" v-show="foldList.includes(match[0].matchDate)">
            <div class="match-name-time">
              ...
              <span class="match-time">{{ dayjs(matchItem.matchDateTime ? matchItem.matchDateTime : matchItem.matchDate
              + ' '+ matchItem.matchTime).format("HH:mm") }}</span>
              <span class="action">
                <span v-if="matchItem.type === 'room'"
                  :class="['icon', 'room', matchItem.matchState === '2' ? 'play' : '']"></span>
                <span v-else :class="['icon', 'event', matchItem.matchState === '2' ? 'play' : '' ]"></span>
                <span>{{ matchItem.matchState == 0 ? $t('暂未开播') : matchItem.type === "room" ? $t('主播讲解'): $t('赛事直播') }}</span>
              </span>
            </div>
            <div class="team-w">
              <div class="team">
        ...
        <div v-show="isEmpty(endObj) && !loading" class="gift-no-data">
          <img src="../../img/quexing.png" alt="">
        </div>
        <Loading :showLoading="loading" class="message-load" />
      </div>
      <div v-else class="gift-list">
        <div v-if="!!giftList.length" class="gift-list-p">
          <div class="tab-item gift-item" v-for="(item, index) in giftList" :key="index">
            <i class="left">
              <span v-if="[0, 1, 2].includes(index)" class="spanp"
                :class="{ 'span0': index === 0, 'span1': index === 1, 'span2': index === 2}"></span>
      ...
      handler (n) {
        ...
        n.map(item => {
          ...
          if (!gameMap[item.matchDate]) {
            ...
            const title = matchDate.format("MM-DD") + ' ' +this.getWeek(matchDate.day());
  ...
  methods: {
    ...
    matchDetail (matchDate) {
      ...
      if (index !== -1) {
        ...
        this.numScrollHeight = -this.endObj[matchDate].length * 109
      }
      else {
        this.foldList.push(matchDate)
        this.numScrollHeight = this.endObj[matchDate].length * 109
      }
    ...
    changeTabIndex (n, o) {
      if (n > o) {
        if (n === 1) {
          this.$refs.tab_list.style.left = '-58px'
        } else if (n > 1) {
          this.$refs.tab_list.style.left = '-116px'
        } else {
          this.$refs.tab_list.style.left = 0
        }
      } else {
        if (n === 4) {
          this.$refs.tab_list.style.left = '-58px'
    ...
    getGiftList () {
      ...
      res.then(r => {
        ...
        if (this.pageNo > 1) {
          this.giftList = this.giftList.concat(data)
        } else {
          this.giftList = data
        }
        this.loading = false
