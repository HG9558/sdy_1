...
<!--        </div>-->
      ...
      <div class="chat-comp">
        <Chat :isShowChat="isShowChat" @changeGiftShow="gitShow = !gitShow" :isShowGift="isShowGift"
          @receiveGift="receiveGift" />
    ...
    <div class="hot-bottom" :class="[isShowChat ? '' : 'not-active']">
      <span @click="isShowChat = !isShowChat"></span>
    ...
    <div class="hot-match-box">
      ...
      <div class="hot-match-list">
        <div class="list-box" :style="{left: -pageIndex * 1080 + 'px'}">
          <div class="hot-match-item" :class="getMatchCls(match)" v-for="(match,index) in list" :key="index"
            @click="changCurMatch(match)">
            ...
            <img v-else class="match-img" :src="match.playUrls[0].streamImgUrl || siteImgMap.matchDefaultBg" alt="" />
  ...
  methods: {
    ...
    async _getMatchList (freshenOnlineNum) {
      ...
      this.list = res.data.data.map(i => {
        i.playUrls = sortSource(i.playUrls, 3, 2)
        return i
      })
    ...
    async changeMatch (match) { //- 获取点击时选取直播频道内容 （热门赛事和直播频道里面的每个直播选项会触发）
      ...
      if (!isCustomize) {
        ...
        let resData = []
        data.vids.map(item => {
          item.playUrls = sortSource(item.playUrls, item.matchType, 2)
          resData.push({
            ...item,
            ...item.playUrls[0]
          })
        })
        data.playUrls = data.playUrls ? sortSource(data.playUrls, data.matchType, 2) : resData
