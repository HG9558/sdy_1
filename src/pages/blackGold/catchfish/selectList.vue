    ...
    <div class="list-wrapper select-list-wrapper">
      <div class="game-type">
        <div class="game-classify">
          <div class="title-box">
            <img class="go-back" @click="$router.push('/catchfish')" src="../electronic/img/goback.png" alt="" />
...
export default {
  ...
  created () {
    !this.selectCatchfishDepotCode && this.$router.push('/catchfish')
  },
  data () {
    return { pageNo: 1 }
  },
  computed: {
    ...
    ...mapState({
      selectCatchfishDepotCode: state => state.game.selectCatchfishDepotCode,
      catchfish: state => state.game.catchfish,
      token: state => state.user.token,
    }),
    renderData () {
      return this.catchfish.filter(item => item.depotCode === this.selectCatchfishDepotCode)[0]
    },
  },
  methods: {
    async _handleCurrentChange (page) {
      this.pageNo = page;
      const res = await this.$http.getGameListByCatIdAndDepotId({ catId: 8, depotId: this.renderData.depotId, page })
