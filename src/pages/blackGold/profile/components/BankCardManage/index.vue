<template>
  <div :class="{'bankcard-manage':true,'wallet':isWallet}">
    <div class="wallet-title">
      <div class="wallet-inner">
        ...
        <div class="wallet-num" v-if="isWallet">
          ...
          <span>{{$t('加密货币钱包：')}}{{ bankcardData.crypto.length }}</span>
          <span>{{$t('支付宝钱包：')}}{{ bankcardData.alipay.length }}</span>
          <span>{{$t('电子钱包：')}}{{ bankcardData.elec.length }}</span>
        </div>
        <div class="wallet-num" v-else>
          <span v-if="tabActive==='rmb'">{{$t('银行卡数量：')}}{{ bankcardData.rmb.length }}/5</span>
          <span v-else-if="tabActive==='crypto'">{{$t('钱包数量：')}}{{ bankcardData.crypto.length }}</span>
          <span v-else-if="tabActive==='elec'">{{$t('钱包数量：')}}{{ bankcardData.elec.length }}</span>
          <span v-else>{{$t('支付宝数量：')}}{{ bankcardData.alipay.length }}</span>
        </div>
      </div>
      ...
      <template v-if="!isWallet">
        <div @click="addBankCard" class="bank-card-list add-bank-card " v-if="(tabActive === 'rmb' && bankcardData[tabActive].length < 5) || (tabActive === 'crypto') || (tabActive === 'alipay') || (tabActive === 'elec')">
          <div class="no-card">
            ...
            <div class="add-btn">{{$t('添加新的提现方式')}}</div>
          </div>
        </div>
      </template>
    ...
    <global-dialog :showDialog.sync="showDialog" :title="$t('新增提现方式')" :hasSelfFooter="true" width="750px">
