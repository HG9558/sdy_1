      ...
      <div class="amount-wrapper">
        <div class="item">
          <div class="amount-inner">
            <div class="title">{{ $t('昨日收益') }}（{{ $tt('currencySymbol') }}）</div>
            <div class="amount">{{ rebateSummary.ydActualReward }}</div>
            <div class="cumulativeAmount">{{ $t('累计收益') }}：{{ $tt('currencySymbol') }}{{ rebateSummary.sumActualReward }}</div>
          </div>
        </div>
        <div class="item">
          <div class="amount-inner">
            <div class="title">{{ $t('昨日首存款返利') }}（{{ $tt('currencySymbol') }}）</div>
            <div class="amount">{{ rebateSummary.ydFirstChargeReward }}</div>
            <div class="cumulativeAmount">
              {{ $t('累计金额') }}：{{ $tt('currencySymbol') }}{{ rebateSummary.sumFirstChargeReward }}
            </div>
          </div>
        </div>
        <div class="item">
          <div class="amount-inner">
            <div class="title">{{ $t('昨日有效投注额返利') }}（{{ $tt('currencySymbol') }}）</div>
            <div class="amount">{{ rebateSummary.ydValidBetReward }}</div>
            <div class="cumulativeAmount">
              {{ $t('累计金额') }}：{{ $tt('currencySymbol') }}{{ rebateSummary.sumValidBetReward }}
            </div>
          </div>
        </div>
        <div class="btns">
