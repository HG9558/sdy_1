      ...
      <div class="upload-inner">
        <el-upload :on-remove="() => (imgData = null)" ref="upload" class="upload-demo" drag :limit="1" action="" :show-file-list="true" :auto-upload="false" :on-change="changeHandle" list-type="picture" accept=".jpg,.png,.jpeg">
          <div class="upload-container" v-if="!imgData">
            ...
            <div class="el-upload__text">{{$t('请将文件拖到此处，或')}}<em style="color: $c-4d">{{$t('点击上传')}}</em></div>
            <div>{{$t('（文件格式为PNG或JPG，且大小不超过15MB，目前支持上传一张）')}}</div>
          </div>
        </el-upload>
      </div>
    </div>
    <el-button :loading="loading" @click="btnClick()" type="button" class="btnCommit" :class="
          selectValue &&
          ((form.desc && form.desc.length && form.desc.length >= 20) ||
            !!imgData)
            ? 'btnCommit-on'
            : ''
        ">{{$t('提交反馈')}}</el-button>
