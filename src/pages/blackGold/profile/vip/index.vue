<template>
  <div class="exclusive-vip">
    <div class="data-box">
      <div class="header">
        <div class="header-left">
          ...
          <img @click="openBirthday" v-else :src="birthdayImg.get" />
        </div>
              ...
              <div class="row-right dark-bg">
                <span>{{$tt('currencySymbol')}}{{parseInt(vipData.depositAmount)}}
                  <template v-if="vipData.activityLevelList && vipData.accountLevel !== vipData.activityLevelList[vipData.activityLevelList.length - 1].accountLevel">
                    / <span class="num">{{ getValidbet('top') }}</span>
                  </template>
                </span>
              </div>
            </div>
            <div class="line-row " v-show="validbetMinShow">
              ...
              <div class="row-right dark-bg">
                <span>{{$tt('currencySymbol')}}{{parseInt(vipData.validbet)}}
                  <template v-if="vipData.activityLevelList && vipData.accountLevel !== vipData.activityLevelList[vipData.activityLevelList.length - 1].accountLevel">
                    / <span class="num">{{ getValidbet('bottom') }}</span>
                  </template>
                </span>
              </div>
            </div>
      ...
      <div class="vip-progress">
        <div class="vip-select">
          <div :key="index" class="vip-bar-text" :class="{ 'vip-bar-text-color': vipBarTxtColorIndex === index }" v-for="(item, index) in vipList" @click="getLevel(item, index)">
            <span class="vip-num">{{ item }}</span>
          </div>
        </div>
        <div class="progress">
          <g-progress :percentage="nowLevelPercentage" :stroke-width="15"></g-progress>
        </div>
      </div>
    </div>
    <div v-if="
      ...
      <div class="row text-center vip-center">
        ...
        <div class="col px-0">
          <div class="font-weight-bolder">
            ...
            <span v-else>{{
              vipData.upgradeBonusLevelDtos && vipData.upgradeBonusLevelDtos[nowVipLevel - 1].donateAmount
            }}</span>
          </div>
          <div class="fs-0-7">{{$t('升级礼金（晋级自动发放）')}}</div>
        </div>
        <div class="col px-0">
          <div class="font-weight-bolder">
              <span v-if="nowVipLevel===0">0</span>
            <span v-else>{{
              nowVipLevel != 0 && vipData.birthdayBonusList[nowVipLevel - 1].donateAmount
                ? vipData.birthdayBonusList[nowVipLevel - 1].donateAmount : 0
            }}</span>
          </div>
          <div class="fs-0-7">{{$t('生日礼金')}}</div>
        </div>
        <div class="col px-0">
          <div class="font-weight-bolder orange">
            <span>{{
              nowVipLevel === 0 ? 0 :vipData.monthlyBonus.ruleScopeDtoList[nowVipLevel - 1].donateAmount
            }}</span>
          </div>
          <div class="fs-0-7">{{$t('每月红包（月初自动发放）')}}</div>
        </div>
      </div>
    </div>
    <div class="data-box vip-box" v-show="
      ...
      <div class="row text-center vip-center" v-if="ruleScopeDtos">
        <div class="col px-0">
      ...
      <div class="row bottom-btn-group">
        <el-select class="bog-select" popper-class="bog" v-model="venueName" :placeholder="$t('查看指定场馆')">
    ...
    <div v-show="birthdayFlog" class="birthday-mask">
      ...
      <div class="birthday-inner-box">
        ...
        <div class="birthday-inner">
          ...
          <div class="div-color bless">{{$tt('currencyValue')}}{{ birthday.donateAmount }}</div>
          <div class="div-while">
...
export default {
  ...
  computed: {
    ...
    birthdayImg() {
      try {
        ...
        return {
          bg: require('./img/birthday-bg_en.png'),
          get: require('./img/birthday-case_en.png'),
        }
      }
    },
    getVipLvlImg () {
  ...
  methods: {
    init () {
      ...
      this.$http.getBirthday().then((res) => {
        if (res.data.code === 0) {
