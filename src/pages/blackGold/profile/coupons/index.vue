  ...
  <div class="coupons">
    <div class="top-div">
      <div class="coupons-inner">
        <div @click="selectClick(item)" :class="{ 'active-bg': selectActive === item.buttonShow }"
          v-for="item in titleArr" :key="item.buttonShow">
          {{ item.name }}
          <div class="dot"></div>
        </div>
      </div>
        ...
        <div class="coupons-wrap-border">
          <div class="coupons-wrap-first">
            ...
            <div class="scoend-div"
              ...
              v-if="!isMixingRule(item)">{{$tt('currencyValue')}}{{ item.amountMax }}</div>
          </div>
          <div class="coupons-wrap-second" v-if="!isMixingRule(item)">
            <span v-if="!isSpecialActive(item)">
              {{$t('领取要求：')}}{{ tmplCode(item).label }}≥{{$tt('currencyValue')}}{{ tmplCode(item).value }}
            </span>
            <span v-if="isSpecialActive(item) && tmplCode(item).value">
              {{ tmplCode(item).label }}:{{$tt('currencyValue')}}{{ tmplCode(item).value }}
            </span>
          </div>
          <div class="coupons-wrap-three">
            ...
            <button @click="nowBtnClick(item)"
              v-show="item.buttonShow !== null && !isMixingRule(item) &&item.tmplCode !=='AQ0000025'" :class="{
            ...
            <template v-if="isMixingRule(item)">
              <div v-for="(_item, _i) in mixingDetail" :key="_i" class="mixing-detailitem">
                <div class="mixing-topbox">
                  <span class="white-color">{{$t('领取要求：')}}{{_item.activityName}}——{{ tmplCode(_item).label }}≥{{$tt('currencyValue')}}{{
                  tmplCode(_item).value }}</span>
                  <span>
                    <span class="scoend-div mixing-title">{{_item.activityName}}</span>
                    <span class="scoend-div mixing-title">{{$tt('currencyValue')}}{{_item.amountMax}}</span>
                    <button @click="openMixingClick(_i)" class="first-btn">
                      <span class="service-manual">{{$t('使用细则')}}</span>
                      <i class="el-icon-arrow-down" v-show="selectMixingDiv === null || selectMixingDiv !== _i"></i>
      ...
      <div v-show="shwoNodata && !dataList.length" class="no-data">
        ...
        <div class="no-data-text">{{$t('暂无优惠记录')}}</div>
      </div>
    ...
    <global-dialog :title="$t('领取提示')" :showDialog.sync="isShowDialog" width="450px" class="unbing-wrap" :hasSelfFooter="true">
      <div class="d-div">
