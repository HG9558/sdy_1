<template>
  <div class="message-wrapper">
    <div class="nav-item-container">
      <div
        :class="{ 'nav-item': true, on: componentName === item.currentType }"
    ...
    <div class="main-container">
      ...
      <div
        class="no-data-img"
        v-if="componentName !== 'TypeOne' && pageTotal === 0 && !showLoading"
      >
      ...
      <Pagination
        ...
        v-if="componentName !== 'TypeOne' && pageTotal !== 0"
...
export default {
  ...
  data() {
    return {
      tabList: [
        { val: this.$t('收件箱'), type: 1, icon: 'message', currentType: 'TypeOne' },
        { val: this.$t('通知'), type: 2, icon: 'inform', currentType: 'TypeTwo' },
        { val: this.$t('公告'), type: 0, icon: 'notice', currentType: 'TypeThree' },
      ],
  ...
  methods: {
    ...
    getUnReadApi(msgType) {
      return this.$http.getMessageUnread({
        msgType
      });
    },
    async getAllUnReadCount() {
      ...
      for(let item of this.tabList){
        if(item.type !== 0){
          let res = await this.getUnReadApi(item.type);
          item['unReadCount'] = res.data.count;
        }
        arr.push(item);
      }
      this.tabList = arr;
    ...
    async _getUnReadCount() {
      const res = await this.getUnReadApi(this.reqData.msgType)
    ...
    switchNextList(data) {
      ...
      if (this.componentName === 'TypeThree') {
        this._getNoticeData()
        return
      }
      this._initMessageData();
