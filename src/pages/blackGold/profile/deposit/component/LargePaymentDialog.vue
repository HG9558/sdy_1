<template>
  <div class="dialog-wrapper">
    <div class="dialog-content" v-for="(item, index) in renderList" :key="index">
      ...
      >
        <img src="../../img/deposit/copy.png" alt="" class="copy-img" />
        {{$t('复制')}}
      </div>
    </div>
    ...
    <div v-else>
      <div>{{ $t('操作步骤:') }}</div>
      <div class="setup">
        <div>{{ $t('1.登录您自己设备上的银行APP或银行官网;') }}</div>
    ...
    <div class="btns">
      ...
      <button class="bog deposit-btn self-right-btn" @click="gotoUrl" type="danger">{{ $t('充值问题需要协助') }}</button>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    renderList() {
      ...
      const data = [{ realName: this.$t('收款姓名：') }, { bankAccount: this.$t('银行卡号：') }, { bankBranch: this.$t('开户网点：') }, { depositAmount: this.$t('充值金额：') }];
