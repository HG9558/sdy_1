  ...
  <div class="cr-pay-container">
    <div v-if="!showLoading && !noData && crChannelList.length">
      <div class="tip">{{$t('选择充值通道')}}
        <span class="usdt-txt" v-if="stationSetting.usdtBuyUrl" @click="hdclick"> <img src="../images/icon-shopcar.svg" alt="">{{$t('购买USDT币')}}</span>
      ...
      <div class="select-pay-form">
        <div class="tip"><span>{{$t('存款金额')}}</span>
        <!-- 存就送icon，勿删 -->
        ...
        <div class="select-box">
          ...
          <div class="select-form-container">
            ...
            <el-button class="handle-btn bog btn-deposit" @click="_cyPayDeposit" :loading="buttonLoading">{{$t('立即存款')}}</el-button>
          ...
          <div class="info-item exchange-num">{{$t('当前参考汇率：')}} 1 USDT ≈ {{$tt('currencyValue')}}{{exchangeRate}}</div>
            <div class="info-item get-money">{{$t('预计到账金额：')}} {{$tt('currencyValue')}}{{userGetMoney}}</div>
          </div>
        </div>
      </div>
    ...
    <global-dialog v-if="crChannelList.length" width="475px" class="cr-dialog" :title="`${crChannelList[selectIndex].crName}(${crChannelList[selectIndex].category})`" :showDialog.sync="showDialog" :hasSelfFooter="false" :cancelText="$t('已完成充值')" :confirmText="$t('充值问题需要协助')" @confirmHandle="confirmHandle">
      <div class="cr-pay-dialog">
        ...
        <div>
          <div class="cr-line">
            ...
            <span class="cr-value">{{$tt('currencyValue')}}{{crPayInfo.depositAmountCNY}}</span>
          </div>
          <div class="cr-line">
          ...
          <div class="cr-line">
            ...
            <span class="cr-button" type="primary" v-clipboard:copy="crPayInfo.walletAddress" v-clipboard:error="onCopyError" v-clipboard:success="onCopySuccess">{{$t('复制')}}</span>
          </div>
        </div>
