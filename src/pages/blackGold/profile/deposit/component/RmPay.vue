  ...
  <div class="cr-pay-container rm">
    <div v-if="!showLoading && !noData">
      ...
      <div class="select-pay-container rm-pay-container">
        <div :class="{'select-pay-item rm-pay-item':true, active:index === payChannelIndex}" @click="changePayChannelIndex(index)" v-for="(item,index) in renderList[payListIndex].children" :key="index">
          ...
          <div class="pay-name">
            ...
            <span class="number">
              {{$tt('currencyValue')}}{{item.amountType === 1 ?
      ...
      <div class="select-pay-form">
       <div class="tip">
         {{$t('存款金额')}}
         <span v-show="['new_bank','new_qq'].includes(renderList[payListIndex].code)">
          ...
          <div class="select-form-container">
            <el-form class="bog rm-form" :model="feeForm" :rules="feeFormRules" ref="feeForm">
              <el-form-item prop="fee">
                <el-input type="number" class="input-money" v-show=" curPayChannel.amountType !== 1" :clearable="true" :placeholder="feePlaceholder" v-model.number="feeForm.fee" ></el-input>
              </el-form-item>
            ...
            <el-button :class="{'handle-btn bog btn-deposit':true , 'rm-handle-btn':curPayChannel.amountType ===1}" @click="_rmPayDeposit" :loading="buttonLoading">{{$t('立即存款')}}</el-button>
    ...
    <global-dialog class="deposit-dialog" :showDialog.sync="showDialog" :title="dialogTitle" :hasSelfFooter="hasSelfFooter" :cancelText="cancelText" :confirmText="confirmText" :btnCancelType="btnCancelType" :btnConfirmType="btnConfirmType" @confirmHandle="confirmHandle" width="600px">
...
export default {
  ...
  data () {
    return {
      ...
      money: null,
      renderList: [
        { label: this.$t('本地银行'), children: [], imageUrl: 'new_bank' },
        { label: this.$t('支付宝'), children: [], imageUrl: 'new_alipay' },
        { label: this.$t('微信'), children: [], imageUrl: 'new_wechat' },
        { label: this.$t('网银支付'), children: [], imageUrl: 'new_unionpay' },
        { label: this.$t('扫码支付'), children: [], imageUrl: 'new_qrcode' },
        { label: this.$t('QQ支付'), children: [], imageUrl: 'new_qq' },
      ],
      bankList: [],
      noticeTextData,
      feeForm: {
        ...
        fastBanks.forEach((item) => {
          if (item.alipayFlg == 0) {
            ...
            item.bankCards.forEach((items) => {
              ...
              items.isHot = item.isHot
              items.isRecommend = item.isRecommend
              items.bankLog = require("../../img/deposit/alipay.png");
              aliList.push(items);
            });
          }
        });
        ...
        if (this.curPayChannel.alipayFlg === 1) {
          try {
            ...
            this.openDialog({
              ...
              dialogTitle: `${this.bankDialogData.bankName}${this.$t('银行卡支付')}`,
