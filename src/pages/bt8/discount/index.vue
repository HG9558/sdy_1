...
export default {
  ...
  created () {
    ...
    if (Object.keys(this.discountData).length) {
      !this.discountData[this.discountType] && this.set_discount_type('全部活动');
      this._getDataList();
    } else {
  ...
  computed: {
    ...
    activeName: {
      get () {
        return (this.discountData[this.discountType] && this.discountType) || '全部活动'
      },
      set (val) { }
    }
  },
  methods: {
    ...
    async _getDiscountTypeList () {
      ...
      res.data.page.forEach(item => {
         item.catName === '全部' && (item.catName = '全部活动');
