        ...
        <transition name="el-fade-in-linear">
          <div v-show="showControlBar" :style="{'z-index': (playerState.tipsStatus === TIPS_STATUS.LOADING || playerState.tipsStatus === TIPS_STATUS.ERROR) ? 99 : 'initial'}" class="player-video_controlBar">
            ...
            <div class="player-video_controlBar_right">
              <el-popover popper-class="player-popover-wrapper popover-right" v-model="showPopover" trigger="click" placement="top-start">
                <div class="player-popover ">
                  <div v-for="(item,index) in playerState.liveStreams" :key="index" class="player-popover-item" :class="{'active': playerState.activeLiveStream.id === item.id}" @click="handleChangeStream(item)">
                    {{item.name}}
                  </div>
                </div>
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      if (this.playerState.liveStreams && this.playerState.liveStreams[0]) {
        const [activeLiveStream] = this.playerState.liveStreams
