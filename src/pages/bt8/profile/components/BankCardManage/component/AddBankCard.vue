    ...
    <div class="add-bank-card-form-wrapper">
      ...
      <div class="tab-container">
        ...
        <div class="tab-main">
          ...
          <!-- <div v-if="alipayEnable" :class="{'money-type':true, on: componentName=== 'AliBank' }" @click="componentName = 'AliBank'">支付宝</div> -->
        </div>
      </div>
      <!--  人民币添加  虚拟币添加 支付包添加 -->
      <keep-alive>
...
export default {
  ...
  watch: {
    ...
    tabActive: {
      immediate: true,
      handler (val) {
        this.componentName = val === 'rmb' ? 'RmbBank' : val === 'alipay' ? 'AliBank' : 'CryBank'
        this.formName = val === 'rmb' ? 'rmbBankForm' : val === 'alipay' ? 'alipayBankForm' : 'cyBankForm'
      }
    },
  },
  data () {
  ...
  methods: {
    ...
    requestFun (form) {
      if (form === 'rmbBankForm') {
        ...
        const params = {
          cardNo: this.$refs.childCom.alipayBankForm.cardNo
        };
          ...
          if (!res.data.code && res.data.msg === "success") {
            this.requestFun(form).then((res) => {
              if (res.data.code === 0) {
                ...
                this.messageTip({ message: form === 'rmbBankForm' ? "银行卡添加成功" : form === 'cyBankForm' ? "USDT钱包添加成功" : '支付宝账户添加成功' });

                form === 'rmbBankForm' ? this._getBankCardList() : form === 'cyBankForm' ? this._getCryptoList() : this._getAlipayList();
              } else {
