    ...
    <div v-if="!showLoading && !noData">
      ...
      <div class="select-pay-container rm-pay-container">
        <div :class="{'select-pay-item rm-pay-item':true,on:index === payChannelIndex}" @click="changePayChannelIndex(index)" v-for="(item,index) in renderList[payListIndex].children" :key="index">
          ...
          <div class="pay-name">
            <span class="text">{{item.bankName || item.showName}}</span>
            <span class="number">
              {{item.amountType ?
      ...
      <div class="user-name-container bog el-form" v-if="curPayChannel.paymentType == 13">
        <span>存款姓名：</span>
      ...
      <div class="select-pay-form">
          <div class="tip">
          存款金额
          <span v-show="renderList[payListIndex].imageUrl === 'new_bank'">
...
export default {
  ...
  data () {
    return {
      ...
      renderList: [
        ...
        { label: "银行卡", children: [], imageUrl: 'new_unionpay' },
        { label: "网银快捷", children: [], imageUrl: 'new_bank' },
      ],
      bankList: [],
      noticeTextData,
      feeForm: {
  ...
  methods: {
    ...
    getPzpayPictureUrl (response) {
      ...
      //- 本地银行
      res.bankTransferList && this.renderList[2].children.push(...res.bankTransferList);
      //- 支付宝
      res.alipayList && res.alipayList.length && this.renderList[0].children.push(...res.alipayList);
      //- 微信添加内容
      res.weChatList && res.weChatList.length && this.renderList[1].children.push(...res.weChatList);
      //-  网银支付
    ...
    async getFastBankList (response) {
      ...
      this.renderList[0].children.unshift(...aliList);
      this.bankList.forEach(item => item.alipayFlg = 1);
      this.renderList[2].children.unshift(...this.bankList);
    ...
    async _rmPayDeposit () {
      ...
      //- 需要输入用户名
      if (this.curPayChannel.paymentType === 13 && !this.accountName) return this.messageTip({ message: "存款人姓名不能为空" });
      ...
      if (this.curPayChannel.alipayFlg === 1) {
        try {
          const res = await this.$http.getPayUrlBank({
            terminal: 0,
            userName: this.curPayChannel.paymentType === 13 ? this.accountName : this.userInfo.realName,
