<template>
  <div class="discount-main ">
    <div class="discount-item-box animate__animated  animate__fadeInRight " v-for="item in list" :key="item.id" @click="openDetail(item)" :class=" { 'out-date':(new Date(item.useEnd.split(' ')[0]) - Date.now()) <= 0}">
      ...
      <div :class="`notice-mark ${item.flag}`">
        <span> {{item | formatDiscountTime}}</span>
        <img :class="item.flag == 'exits'?'long':'normal'" :src="require(`../img/${item.flag == 'exits' ? 'long_bg' : 'time_bg'}.png`)" alt="">
      </div>
    </div>
  </div>
</template>
