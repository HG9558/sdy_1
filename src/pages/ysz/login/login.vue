<template>
  <Layout>
    <div class="form-container">
      ...
      <template v-if="isAccountLogin">
        <el-form :model="accountLoginForm" :rules="accountLoginRule" ref="accountLoginForm" key="accountLoginForm">
          <el-form-item prop="loginName">
            <el-input class="user-input" prefix-icon="el-icon-user" v-model="accountLoginForm.loginName" :maxlength="16" clearable placeholder="用户名">
            </el-input>
          </el-form-item>
          <el-form-item prop="password">
            <el-input class="user-input" prefix-icon="el-icon-lock" v-model.trim="accountLoginForm.password" type="password" :maxlength="18" clearable @keyup.enter.native="_dataFormSubmitHandle('accountLoginForm')" placeholder="密码">
            </el-input>
          </el-form-item>
      ...
      <template v-else>
        <el-form class="mobile-login-form" :model="mobileLoginForm" :rules="mobileLoginRule" ref="mobileLoginForm" key="mobileLoginForm">
          <el-form-item :rules="activeMobileAreaCfg.rules" class="mobile-select" prop="mobile">
            ...
            <el-input prefix-icon="el-icon-mobile-phone" class="mobile-input" v-model="mobileLoginForm.mobile" :maxlength="activeMobileAreaCfg.maxlength" clearable placeholder="手机号">
            </el-input>
          </el-form-item>

          <el-form-item class="code-input" prop="mobileCaptcha">
            <el-input v-model="mobileLoginForm.mobileCaptcha" prefix-icon="el-icon-timer" :maxlength="6" placeholder="短信验证码">
      ...
      <el-button class="bog login-btn" :disabled="btnDisabled" @click="loginBtnClick" :loading="isLoading" @keydown.native.enter="loginBtnClick">
        <span class="ft-16" style="cursor: pointer;">登录</span>
      </el-button>

      <div class="goto-other">
        <span class="c-r" @click="changeLoginType">{{isAccountLogin ? '手机号登录' : '账号登录'}}</span>
        <span class="c-r register-new-account"><span @click="$router.push('/login/register')">注册新账号</span>
...
.el-button {
  ...
  &:hover,
  &:active {
    background-color: $c-fcc !important;
    border-color: $c-fcc !important;
  }
}
::v-deep .code-input {
  .el-input__inner {
