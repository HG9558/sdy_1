  ...
  <div class="my-friend-detail-wrapper">
      ...
      <div class="account-info">
        <div class="account-info_left">
          ...
          <div>
            ...
            <div class="logo-date">
              最近登陆：{{ friendDetailData.lastLoginTime }}
            </div>
          </div>
        </div>
        <div class="account-info_right">
          <div class="box">
            ...
            <div>¥{{ friendDetailData.firstCharge || "0.00" }}</div>
          </div>
          <div class="box">
            <div class="box-title">累计充值</div>
            <div>¥{{ friendDetailData.totalDeposit || "0.00" }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="statistics-time">
      ...
      <div class="date-search">
        <el-date-picker popper-class="black-bg" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" v-model="timeList"></el-date-picker>
      ...
      <div class="date-box">
        <div v-for="(item, index) in dateList" :key="index" @click="chooseBtn(item, index)" :class="{ btn: true, on: currentIndex === index }">
          {{ item }}
        </div>
      </div>
      ...
      <div class="record-list-container">
        <div :class="`record-list-item record-list-item-${index+1}`" v-for="(item, index) in recordList" :key="index">
          <span class="record-list-text"> {{ item.txt }} (元) </span>
