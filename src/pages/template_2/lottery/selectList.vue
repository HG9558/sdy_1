    ...
    <div class="list-wrapper select-list-wrapper">
      <div class="game-type">
        <div class="game-classify">
          <div class="title-box">
            <img class="go-back" @click="$router.push('/lottery')" :src="useImgPath('electronic/goback.png')" alt="" />
            <img
              class="logo"
              :src="useImgPath(`lottery/logo_${renderData.depotCode}.png`)"
              @error="$event => useErrorPath('lottery/logo_default.png', $event)" />
            <div class="title">
              <div>
                {{ renderData[`${globalState.language}_txt` as keyof API.normalGameType] }}
              </div>
...
onMounted(() => {
  !gameState.selectLotteryDepotCode && router.push('/lottery');
});

const renderData = computed(() => {
  return gameState.lottery.filter(item => item.depotCode === gameState.selectLotteryDepotCode)[0];
});

const _handleCurrentChange = async ({ current }: { current: number }) => {
  pageNo.value = current;
  const res = await api.game.getGameLotteryList({
    depotId: renderData.value.depotId,
    page: current,
  });
