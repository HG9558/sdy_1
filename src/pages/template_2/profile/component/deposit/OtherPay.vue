  ...
  <div :class="['cr-pay-container', site, noData && 'bg']">
    <template v-if="!showLoading && !noData">
      ...
      <div class="bg-container">
        ...
        <div class="user-name-container el-form" v-if="curPayChannel?.paymentType == 17 && curPayChannel.depositName">
          <span>{{ $t_c('存款姓名：') }}</span>
...
const renderList = reactive<DepositType.BankRenderListType[]>([
  { label: 'EBPAY', children: [], imageUrl: 'new_ebpay', code: 'ebList' },
  { label: 'TOPAY', children: [], imageUrl: 'new_topay', code: 'toList' },
  { label: lang('钱包转账'), children: [], imageUrl: 'new_wallet', code: 'walletList' },
  { label: "GOPAY", children: [], imageUrl: 'new_gopay', code: 'goList' },
]);

onMounted(() => _initOtherPay());

const _initOtherPay = async () => {
  const res = await api.deposit.getPzpayPictureUrl();
  getPzpayPictureUrl(res);
...
const getPzpayPictureUrl = (response: DepositType.getPzpayPictureUrlRes) => {
  ...
  res.ebpayList && res.ebpayList.length && renderList[0].children.push(...res.ebpayList);
  res.topayList && res.topayList.length && renderList[1].children.push(...res.topayList);
  res.otherWalletPayList && res.otherWalletPayList.length && renderList[2].children.push(...res.otherWalletPayList);
  res.gopayList && res.gopayList.length && renderList[3].children.push(...res.gopayList);
  ...
  userState.modify_depositLen({ type: 'OtherPay', len: renderList.length });
...
const _rmPayDeposit = async () => {
  ...
  if (curPayChannel.value.paymentType === 17 && curPayChannel.value.depositName && !accountName.value)
    ...
    if (curPayChannel.value.alipayFlg === 1) {
      try {
        const res = await api.deposit.getPayUrlBank({
          terminal: 0,
          userName:
            curPayChannel.value.paymentType === 17 && curPayChannel.value.depositName
