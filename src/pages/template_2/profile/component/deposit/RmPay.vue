  ...
  <div :class="['cr-pay-container', site, noData && 'bg']">
    <template v-if="!showLoading && !noData">
      <div class="pay-type-container">
        <div
          :class="{ 'py-type-list': true, on: index === payListIndex }"
          @click="changePayListIndex(index)"
          v-for="(item, index) in renderList"
          :key="index">
          <div class="circle-outer">
            <p v-if="item.isShow && item.isHot" :class="['tag', 'hot', globalState.language]">
              <span class="icon_tag"> {{ $t_c('热门') }}</span>
            </p>
            <p v-if="item.isShow && !item.isHot && item.isRecommend" :class="['tag', globalState.language]">
              <span class="icon_tag"> {{ $t_c('推荐') }}</span>
            </p>
            <div class="circle-inner">
              <img
                :class="`list-img logo_${index + 1}`"
                :src="useImgPath(`profile/deposit_${item.imageUrl}.png`)"
                alt="" />
            </div>
          </div>
      ...
      <div class="bg-container">
        ...
        <div class="select-pay-container rm-pay-container">
          <div
            ...
            v-for="(item, index) in renderList[payListIndex]?.children"
            :key="index">
            <img
              class="logo"
              :src="item.bankLog || item.bankLogo || useImgPath('profile/deposit_bankcard3x.png')"
              alt="" />
            <div class="pay-name">
        ...
        <div class="user-name-container el-form" v-if="curPayChannel?.paymentType == 13 && curPayChannel.depositName">
          <span>{{ $t_c('存款姓名：') }}</span>
        ...
        <div class="select-pay-form">
          <div class="tip_1">
            ...
            <span
              v-show="
                ['new_bank', 'new_qq', `${envState.vite_currency_map.type.toLowerCase()}_new_bank`].includes(
                  renderList[payListIndex].imageUrl
                )
              ">
              <span class="tip-text-t"
                ...
                {{
                  renderList[payListIndex]?.children[payChannelIndex]?.amountType
                    ? `${renderList[payListIndex].children[payChannelIndex].fixedAmount.split(',')[0]}-${renderList[
                        payListIndex
                      ].children[payChannelIndex].fixedAmount
                        .split(',')
                        .pop()} `
          ...
          <div class="select-box rm-box">
            ...
            <div class="pay-tip-text">
              {{
                renderList[payListIndex].code === 'bankList'
                  ? noticeTextData[renderList[payListIndex].code as keyof typeof noticeTextData]
                  : noticeTextData[renderList[payListIndex].code as keyof typeof noticeTextData] ??
                    noticeTextData[renderList[payListIndex].label as keyof typeof noticeTextData] +
    ...
    <self-dialog
      class="bg-dialog-body"
      :dialogTitle="dialogTitle"
      v-model:isShow="showDialog"
      :confirmTxt="$t_c('确认')"
      :showCancel="false"
      :width="700"
      @confirmCallback="() => (showDialog = false)">
...
const payListIndex = ref<number>(0); //- 银行分类
...
const bankDialogData = reactive<DepositType.applyPayBankInfo>({} as DepositType.applyPayBankInfo); //- 大额支付专用
const dialogTitle = ref<string>(lang('温馨提示'));
const showDialog = ref<boolean>(false);
const site = useSite();
const globalState = useGlobalState();
const envState = useEnvState();

const renderMap: DepositType.RenderMapType = {
  RMB: [
    { label: lang('本地银行'), children: [], imageUrl: 'new_bank', code: 'bankList' },
    { label: lang('支付宝'), children: [], imageUrl: 'new_alipay', code: 'alipayList' },
    { label: lang('微信'), children: [], imageUrl: 'new_wechat', code: 'weChatList' },
    { label: lang('网银支付'), children: [], imageUrl: 'new_unionpay', code: 'wyList' },
    { label: lang('扫码支付'), children: [], imageUrl: 'new_qrcode', code: 'qrCodeList' },
    { label: lang('QQ支付'), children: [], imageUrl: 'new_qq', code: 'qqList' },
    { label: lang('数字人民币'), children: [], imageUrl: 'ecny', code: 'ECNYList' },
  ],
  VN: [
    {
      label: lang('本地银行'),
...
const renderList = reactive<DepositType.BankRenderListType[]>(renderMap[envState.vite_currency_map.type]);
onMounted(() => _initRmb());

const _initRmb = async () => {
  const resList = await Promise.allSettled([
    api.deposit.getPzpayPictureUrl(),
    api.deposit.getBankList(),
  ...
  resList[1].status === 'fulfilled' && getBankList(resList[1].value);
  resList[2].status === 'fulfilled' && getFastBankList(resList[2].value);
  showLoading.value = false;
  noData.value = !renderList.length;
};

const getPzpayPictureUrl = (response: DepositType.getPzpayPictureUrlRes) => {
  const { code, msg, res, payTypeList } = response;

  if (code) return messageTip({ message: msg });

  //- 越南单独处理
  envState.vite_currency_map.type === 'VN' &&
    res.eeziePayPayList &&
    renderList.find(_ => _.code === 'eeziePayList')?.children.push(...res.eeziePayPayList);

  //- 本地银行
  res.bankTransferList &&
    res.bankTransferList.length &&
    renderList.find(_ => _.code === 'bankList')?.children.push(...res.bankTransferList);

  //- 支付宝
  res.alipayList &&
    res.alipayList.length &&
    renderList.find(_ => _.code === 'alipayList')?.children.push(...res.alipayList);

  //- 微信添加内容
  res.weChatList &&
    res.weChatList.length &&
    renderList.find(_ => _.code === 'weChatList')?.children.push(...res.weChatList);
  //-  网银支付
  res.wyList && res.wyList.length && renderList.find(_ => _.code === 'wyList')?.children.push(...res.wyList);

  //- 扫码支付
  if (res.qrCodeList && res.qrCodeList.length) {
    res.qrCodeList.forEach((item: DepositType.QrCodeList) => (item.payType = 'qr'));
    renderList.find((_: DepositType.BankRenderListType) => _.code === 'qrCodeList')?.children.push(...res.qrCodeList);
  }

  //- QQ支付
  res.qqList &&
    res.qqList.length &&
    renderList.find((_: DepositType.BankRenderListType) => _.code === 'qqList')?.children.push(...res.qqList);

  //- 数字人民币
  res.ecnyPayList &&
    res.ecnyPayList.length &&
    renderList
      .find((_: DepositType.BankRenderListType) => _.code === 'ECNYList')
      ?.children.push(...(res.ecnyPayList as DepositType.EcnyPayList[]));

  if (payTypeList && payTypeList.length) {
    renderList.forEach((item: DepositType.BankRenderListType) => {
      const pickRes = payTypeList.filter(itemPayType => itemPayType.code === item.code);
      if (pickRes && pickRes.length) {
        item.isShow = pickRes[0].isShow;
        item.isHot = pickRes[0].isHot;
        item.isRecommend = pickRes[0].isRecommend;
      }
    });
  }
};
    ...
    fastBanks.forEach((item: DepositType.FastBank) => {
      if (+item.alipayFlg == 0) {
        ...
        item.bankCards.forEach(items => {
          ...
          items.bankLog = useImgPath('profile/deposit_rm_alipay.png');
          aliList.push(items);
        });
      } else if (+item.alipayFlg == 2) {
        item.bankCards.forEach(items => {
          items.alipayFlg = 2;
          items.bankName = item.showName;
          items.amountType = item.amountType;
          ...
          items.isHot = item.isHot;
          items.isRecommend = item.isRecommend;
          items.bankLog = useImgPath('profile/deposit_ecny.png');
          renderList.find(_ => _.code === 'ECNYList')?.children.unshift(items);
        });
      }
    });
  }

  renderList.find(_ => _.code === 'alipayList')?.children.unshift(...aliList);
  bankList.forEach((item: DepositType.BankCard | DepositType.fastBankListBankCard) => (item.alipayFlg = 1));
  renderList.find(_ => _.code === 'bankList')?.children.unshift(...bankList);
  renderList.filter(_ => _.children.length);

  for (let index = 0; index < renderList.length; index++) {
    if (!renderList[index].children.length) {
      renderList.splice(index, 1);
      index--;
    }
  }

  userState.modify_depositLen({ type: 'RmPay', len: renderList.length });
  const { minAmout, maxAmout } = renderList[0]?.children[0] ?? { minAmout: 0, maxAmout: 0 };
  feePlaceholder.value = `${minAmout}-${maxAmout}`;
};

const curPayChannel = computed(() => {
  return renderList[payListIndex.value].children[payChannelIndex.value];
});

//- 充值类型切换
const changePayListIndex = (index: number) => {
  payListIndex.value = index;
  recommendMoneyIndex.value = -1;
  payChannelIndex.value = 0;
...
const _rmPayDeposit = async () => {
  ...
  if (curPayChannel.value.paymentType === 13 && curPayChannel.value.depositName && !accountName.value)
...
const _onlinePayDeposit = async () => {
  ...
  if (curPayChannel.value.payType === 'qr') {
    //- 扫码支付
    const res = await api.deposit.applyQrPay({
      fee: feeForm.fee as number,
      depositId: curPayChannel.value.id as number,
    });
    ...
    if (curPayChannel.value.alipayFlg === 1) {
      try {
        const res = await api.deposit.getPayUrlBank({
          terminal: 0,
          userName:
            curPayChannel.value.paymentType === 13 && curPayChannel.value.depositName
              ? (accountName.value as string)
              : (userState.userInfo.realName as string),
          fee: feeForm.fee as number,
          depositId: curPayChannel.value.id as number,
        });
        feeFormRef.value?.resetFields();
        ...
        Object.keys(bankDialogData).forEach(key => delete bankDialogData[key]);
        Object.assign(bankDialogData, res.info);

        dialogTitle.value = `${bankDialogData.bankName}${lang('大额支付专用')}`;
        confirmText.value = lang('充值问题需要协助');
        cancelText.value = lang('已完成充值');
        showDialog.value = true;
      } catch (error) {
        console.log('560行打印：============:', error);
        buttonLoading.value = false;
        feeFormRef.value?.resetFields();
      }
    } else {
      ...
      validator: (_: any, value: string, callback: Function) => {
        ...
        if (curPayChannel.value) {
          if (!value) {
            callback(
              new Error(
                `${lang('请')}${curPayChannel.value.amountType === 1 ? lang('选择') : lang('输入')}${lang('存款金额')}`
              )
            );
          }
          if (!reg.test(value)) {
