<template>
  <special-layout
    ...
    <img
      :class="`cockfighting-main-pic ${renderList[currentSelectIndex].depotCode}`"
      :src="useImgPath(`special/cockfighting_${currentSelectIndex + 1}.png`)" />
  </special-layout>
</template>
...
const renderList = computed(() => {
  if (!gameState.cockfighting.length) return [];
  return JSON.parse(JSON.stringify(gameState.cockfighting)).map((item: API.normalGameType) => {
