<template>
  <div :class="['electronic-partList-wrapper', site]">
    <div class="list-wrapper" v-for="gameItem in gameState.fish" :key="gameItem.depotCode">
      <div class="game-type">
        <div class="game-classify">
          <div class="title-box">
            <img
              class="logo"
              :src="useImgPath(`fish/logo_${gameItem.depotCode.toLowerCase()}.png`)"
              @error="($event:Event)=> useErrorPath('electronic/logo_default.png',$event)" />
            <div class="title">
              <div>{{ gameItem[`${globalState.language}_txt` as keyof typeof gameItem] }}</div>
...
const goAllList = (depotCode: string) => {
  gameState.set_fish_depotCode(depotCode);
