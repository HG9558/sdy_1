    ...
    <div class="list-wrapper select-list-wrapper">
      <div class="game-type">
        <div class="game-classify">
          <div class="title-box">
            <img class="go-back" @click="$router.push('/poker')" :src="useImgPath('electronic/goback.png')" alt="" />
            <img
              class="logo"
              :src="useImgPath(`poker/logo_${renderData.depotCode}.png`)"
...
onMounted(() => {
  !gameState.selectPokerDepotCode && router.push('/poker');
});

const renderData = computed(() => {
  return gameState.poker.filter(item => item.depotCode === gameState.selectPokerDepotCode)[0];
});

const _handleCurrentChange = async ({ current }: { current: number }) => {
  pageNo.value = current;
  const res = await api.game.getGameChessList({
