...
const _getData = async () => {
  const tempArr = JSON.parse(JSON.stringify(gameState.poker));
  const list = await Promise.all(
    tempArr.map((item: { depotId: number }) => {
      return api.game.getGameChessList({ depotId: item.depotId });
    })
  );
  ...
  gameState.set_poker(tempArr);
};

watch(
  () => normalState.renderPokerGame,
  _ => {
    if (_) _getData();
  },
  { immediate: true }
...
watch(
  ...
  (_, pre) => {
    pre.name === 'poker-selectList' && _getData();
    nextTick(() => {
...
onUnmounted(() => {
  normalState.set_renderPokerGame(false);
});

const globalState = useGlobalState();
const bannerBg = `url(${useImgPath(`poker/${globalState.language !== 'zh' ? 'banner_on' : 'banner'}.jpg`)})`;
</script>
