<template>
  <div class="match-comp">
    <div class="tabs">
      ...
      <div class="arrow-right" :class="{'disabled': tabIndex + 1 >= tabs.length}" @click="tabIndex += 1"> </div>
    </div>
    ...
    <el-scrollbar wrap-class="list-wrapper" ref="scrollBar">
      <div class="match-list" v-if="tabIndex !== 5">
        <div
          ...
          v-for="(match, key) in endObj"
          :key="key"
        >
          <div
            v-if="match[0].dateTitle"
            class="date-title"
            @click="matchDetail(match[0].matchDate)"
          >
            {{ match[0].dateTitle }}
            <div class="match-detail-show">
              <img
                v-if="!foldList.includes(match[0].matchDate)"
                src="../../img/live/down.svg"
                alt=""
                srcset=""
              />
          ...
          <div
            @click="handleChangeLive(matchItem)"
            :class="[
              'no-date-title',
              { playing: matchItem.matchId === matchId },
            ]"
            v-for="(matchItem, index) in match"
            :key="matchItem.groupId + index"
            v-show="foldList.includes(match[0].matchDate)"
          >
            <div class="match-name-time">
              ...
              <span class="action">
                <span
                  v-if="matchItem.type === 'room'"
                  :class="[
                    'icon',
                    'room',
                    matchItem.matchState === '2' ? 'play' : '',
                  ]"
                ></span>
                <span
                  v-else
            ...
            <div class="team-w">
              <div class="team">
                <img
                  class="img-box"
                  :src="matchItem.homeLogo || defaultImg"
                  alt=""
                />
              ...
              <div class="team">
                <img
                  class="img-box"
                  :src="matchItem.awayLogo || defaultImg"
                  alt=""
                />
        ...
        <p
          class="no-more"
          v-show="pageTotal && pageNo >= Math.ceil(pageTotal / pageSize)"
        >
          没有更多数据了～
        </p>
      ...
      <div v-else class="gift-list">
        <div class="gift-list-p" v-if="!!giftList.length">
          <div
            class="tab-item gift-item"
...
export default {
      ...
      handler(n) {
        ...
        n.map((item) => {
          ...
          if (!gameMap[item.matchDate]) {
            ...
            const matchDate = item.matchDateTime
              ? dayjs(item.matchDateTime)
              : dayjs(item.matchDate);
            const title =
              matchDate.format("MM-DD") +
              " 星期" +
              this.getWeek(matchDate.day());
  ...
  methods: {
    ...
    handleArrowRight() {
      ...
      console.log(this.$refs.tab_list.style);
    },
    changeTabIndex(n, o) {
      if (n > o) {
        if (n === 1) {
    ...
    getGiftList() {
      ...
      const res = this.weekGiftLog
        ? this.$http.getWeekContributionList(params)
        : this.$http.getTotalContributionList(params);
      const data = res.data.data;
      if (this.pageNo > 1) {
