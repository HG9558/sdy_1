    ...
    <div class="fee-form">
      <el-form class="bog" :model="feeForm" :rules="feeFormRules" ref="feeForm">
        <el-form-item prop="fee" class="fee">
          <el-input class="money-form" :clearable="true" :placeholder="`最低${minAmount}元，最高${maxAmount}元`" v-model.number="feeForm.fee" type="number" step = "0.1"></el-input>
      ...
      this.$refs["feeForm"].validate((valid) => {
        if (valid) {
          ...
          this.openConfirmDialog({
            content: ["您当前尚有彩金未领取，是否放弃领取"],
            cancelButtonText: "放弃",
            confirmButtonText: "领取",
            callback: () => this.$router.push('/profile/coupons'),
