<template>
  <div class="cr-pay-container rm">
    ...
    <global-dialog class="deposit-dialog" :showDialog.sync="showDialog" :title="dialogTitle" :hasSelfFooter="hasSelfFooter" :cancelText="cancelText" :confirmText="confirmText" @confirmHandle="confirmHandle" width="600px">
      <LargePaymentDialog :bankDialogData="bankDialogData" :curRmbChannel="renderList[payListIndex]" :curPayChannel="curPayChannel" />
    </global-dialog>
...
export default {
  ...
  methods: {
    ...
    async _initRmb () {
      const resList = await Promise.allSettled([this.$http.getPzpayPictureUrl(), this.$http.getBankList(), this.$http.getFastBankList({ areaCode: 'AUD' })])
    ...
    async getFastBankList (response) {
      const resPzpayPictureUrl = await this.$http.getPzpayPictureUrl()
      let payTypeList = []
      if (resPzpayPictureUrl && resPzpayPictureUrl.data) {
        payTypeList = resPzpayPictureUrl.data.payTypeList;
      }
      ...
      if (this.curPayChannel.payType === 'qr') {
        ...
        if (this.curPayChannel.alipayFlg === 1) {
          try {
            ...
            //- 大额支付专用
            this.openDialog({ componentName: "LargePaymentDialog", dialogTitle: `${this.bankDialogData.bankName}支付`, cancelText: "已完成充值", confirmText: "充值问题需要协助" })
          } catch (error) {
            console.log(error)
            this.buttonLoading = false;
            this.$refs["feeForm"].resetFields();
