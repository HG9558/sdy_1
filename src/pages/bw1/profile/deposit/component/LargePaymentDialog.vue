<template>
  <div class="dialog-wrapper">
    ...
    <!-- <div class="btns">
      <el-button class="yszjl deposit-btn self-left-btn" type="success" @click="">已完成充值</el-button>
      <el-button class="yszjl deposit-btn self-right-btn" @click="gotoUrl" type="danger">充值问题需要协助</el-button>
    </div> -->
  ...
  computed: {
    renderList () {
      const temp = JSON.parse(JSON.stringify(this.bankDialogData))
			let data = [{ bankAccount: '银行卡号：' }, { depositAmount: '充值金额：' }, { depositPostscript: ['附&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;言：', '(必须正确填写)'] }]
			if (this.isPAYID || (this.curRmbChannel && this.curRmbChannel.label === '支付宝')) {
				data = [ { realName: '收款姓名：' }, ...data, ];
			} else {
				data = [ { realName: '收款姓名：' }, { bankBranch: 'BSB码：' }, ...data, ];
			}
