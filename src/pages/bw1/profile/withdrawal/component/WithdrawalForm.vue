<template>
  <div class="form-container">
      ...
      <div class="money-notice-text">
        ...
        <div class="masker-container" v-if="!isPassed">
          ...
          <div class="notice-msg">
            提现所需有效投注总流水为{{ depositValidBet }}A$，已完成{{
    ...
    <div class="fee-form">
      <el-form class="yszjl" :model="feeForm" :rules="feeFormRules" ref="feeForm">
        <el-form-item prop="fee" class="fee">
          <el-input class="money-form" :clearable="true" :placeholder="`最低${minAmount}A$，最高${maxAmount}A$`"
            v-model.number="feeForm.fee" type="number" step="0.1"></el-input>
          <el-button class="yszjl fee-submit" :disabled="!isPassed" @click="feeFormSubmit" :loading="withdrawalLoading">
            确认提现</el-button>
          <span class="ml-10">可提现金额：{{(!isPassed ? 0 : centerBalanceMoney)}}A$，不包括游戏平台未转出余额</span>
        </el-form-item>
      </el-form>
      ...
      <div class="virtual-text-container" v-if="tabActive==='crypto'">
        当前参考汇率： 1 USDT ≈ {{ exchangeRate }} A$ 预计到账金额：{{ userGetMoney }} USDT
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    feePlaceholder () {
      return `最低${this.minAmount}A$，最高${this.maxAmount}A$`
    },
    userGetMoney () {
