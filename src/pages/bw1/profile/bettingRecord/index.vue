         ...
         <div class="head-left-top">记录查询 <span>(当前系统支持查询最近45日的投注记录)</span></div>
          <div class="head-left-bottom">
            ...
            <el-form class="yszjl betting-form" ref="form" :model="form">
              <el-form-item>
                <el-select popper-class="yszjl" @change="getTableList" v-model="selectValue" placeholder="请选择需要查询的平台">
    ...
    <div class="bottom-div" style="position: relative;">
      <div class="bottom-div-inner">
            ...
            <template slot-scope="props">
              <div v-if="!props.row.odds || props.row.odds.indexOf(',') === -1">
                <el-form label-position="left" inline class="demo-table-expand">
                  <el-form-item label="订单号" style="width: 235px;">
                    ...
                    <span style="margin-left: 10px; color: #505679; cursor: pointer;float: right;" v-clipboard:copy="props.row.id" v-clipboard:success="firstCopySuccess" v-clipboard:error="firstCopyError">复制</span>
                  </el-form-item>
                  ...
                  <el-form-item v-if="props.row.playType" style="text-align: center;" label="玩法类型">
                  ...
                  <el-form-item v-if="props.row.resultScore" style="text-align: center;" label="赛果比分">
                    <span>{{ props.row.halfResultScore ? `半(${props.row.halfResultScore})  全(${props.row.resultScore})`: props.row.resultScore }}</span>
                  </el-form-item>
              ...
              <div v-else>
                ...
                <el-form label-position="left" inline class="demo-table-expand" v-for="(item, index) in props.row.odds.split(',')" :key="'chuanguan-'+index" style="border-top: 1px solid rgba(255, 255, 255, 0.3)">
                  <el-form-item v-if="props.row.odds && props.row.odds.split(',')[index]" :label="props.row.odds ? '赔率' : ''" style="">
                  ...
                  <el-form-item v-if="props.row.resultScore && props.row.resultScore.split(',')[index]" style="text-align: center;" label="赛果比分">
                    <span>{{ props.row.halfResultScore.split(',')[index] ? `半(${props.row.halfResultScore.split(',')[index]})  全(${props.row.resultScore.split(',')[index]})`: props.row.resultScore.split(',')[index] }}</span>
                  </el-form-item>
        ...
        <div class="no-data-gif" v-show="showLoding">
          <img width="300" src="../img/load.gif" alt="" />
        </div>
      </div>
