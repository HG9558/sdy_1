<template>
  <section ref="playerWrapper" class="player-wrapper">
    <div class="player-content" @click="handleChangeShowControlBar($event)">
        ...
        <transition name="el-fade-in-linear">
          <div v-show="showControlBar" :style="{'z-index': (playerState.tipsStatus === TIPS_STATUS.LOADING || playerState.tipsStatus === TIPS_STATUS.ERROR) ? 99 : 'initial'}" class="player-video_controlBar">
            ...
            <div class="player-video_controlBar_right">
              ...
              <el-popover style="margin-left: 10px" popper-class="player-popover-wrapper popover-right" v-model="showPopover"  trigger="click" placement="top">
                <div class="player-popover">
                  <div v-for="item in playerState.activeLiveStream.playUrls" :key="item.id" class="player-popover-item p-h-10" :class="{'active': playerState.activeLiveStream.id === item.id}" @click="handleChangeStream({ ...playerState.activeLiveStream, ...item })">
                    {{item.name}}
                  </div>
                </div>
      ...
      <div v-show="playerState.tipsStatus !== null" class="player-video__empty" :class="{'no-live': !activeMatchData, 'yszjl': templateIsBog}">
