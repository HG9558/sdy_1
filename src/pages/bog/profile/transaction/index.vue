<template>
  <div>
    ...
    <div class="bottom-div">
      ...
      <component :is="componentsNameList[activeSpan]" :time="remindersTime"  @getDataList="getDataList" :tableData="tableData"></component>
...
export default {
  ...
  data () {
    return {
      ...
      componentsNameList: ['deposit', 'withdrawal', 'reward', 'discount', 'rebate'],
      paginationTotal: 0,
      timeList: getTime("今天"),
      totalData: {},
      value1: [
  ...
  created () {
    const transactionItem = sessionStorage.getItem('transactionItem')
    try {
      if (transactionItem && JSON.parse(transactionItem).mark) {
        this.changeSpan(JSON.parse(transactionItem))
      }
    } catch (e) {
      console.log('e:', e)
    }
  ...
  methods: {
    ...
    getDataList () {
      const transactionItem = sessionStorage.getItem('transactionItem')
      if (transactionItem) {
        const transactionItemObj = JSON.parse(transactionItem)
        if (transactionItemObj.mark !== this.activeSpan) {
          return
        }
      }
      this.tableData = []
      ...
      if ((this.activeSpan === 0 || this.activeSpan === 2) && !transactionItem) {
    ...
    changeSpan (item) {
      ...
      this.getDataList();
      sessionStorage.setItem('transactionItem', '')
    },
