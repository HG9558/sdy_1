<template>
  <div class="form-container">
    ...
    <div class="fee-form">
      <el-form class="bog" :model="feeForm" :rules="feeFormRules" ref="feeForm">
        <el-form-item prop="fee" class="fee">
          <div v-if="tabActive === 'paybus'" class="select-card-container">
            <div
              :class="{'select-item rm-select-item':true, on:recommendPaybusMoneyIndex === index, 'last-line': (index + 1) % 5 === 0}"
              v-for="(item,index) in paybusChargeValues" @click="changePaybusFeeMoneyIndex(index)" :key="index">
              {{item}}元
            </div>
          </div>
          <el-input v-else class="money-form" :clearable="true" :placeholder="`最低${minAmount}元，最高${maxAmount}元`"
          ...
          <el-button class="bog fee-submit" :disabled="!isPassed" @click="feeFormSubmit" :loading="withdrawalLoading">
            确认提现</el-button>
          <span class="ml-10">可提现金额：{{(!isPassed ? 0 : centerBalanceMoney)}}元，不包括游戏平台未转出余额</span>
        </el-form-item>
      </el-form>
    ...
    <!--提款申请-->
    <global-dialog :showDialog.sync="showDialog" title="提款申请" :hasSelfFooter="true" width="700px">
      <!--<AddBankCard :tabActive="tabActive" :showDialog.sync="showDialog" />-->
      <PaybusStep :showDialog.sync="showDialog" :paybusData="paybusData" :step="1" />
    </global-dialog>
  </div>
</template>
...
import PaybusStep from "./PaybusStep"
...
export default {
  ...
  watch: {
    'feeForm.fee': {
      deep: true,
      handler (val) {
        this.recommendPaybusMoneyIndex = this.paybusChargeValues.findIndex(item => Number(item) === val)
      }
    },
    tabActive: {
      immediate: true,
      handler (val) {
        ...
        if (val === 'crypto' && (this.bankcardData[val][0].available == 0 || this.bankcardData[val][0].available == 2)) {
          ...
          this.curBankCard = this.bankcardData[val === 'paybus' ? 'rmb' : val][0];
        }
      }
    },
    selectBank: {
  ...
  methods: {
    ...
    changePaybusFeeMoneyIndex (index) {
      this.recommendPaybusMoneyIndex = index;
      this.feeForm.fee = Number(this.paybusChargeValues[index]);
    },
    async _handleWithdrawal () {
      this.withdrawalLoading = true;
      let sendData = null;
      if (this.tabActive === "paybus") {
        sendData = {
          methodType: 4,
          bankCardId: this.curBankCard.id,
          drawingAmount: this.feeForm.fee,
        };
      } else if (this.tabActive === "rmb") {
        sendData = {
          bankCardId: this.curBankCard.id,
          drawingAmount: this.feeForm.fee,
        };
      } else if (this.tabActive === "ecny") {
        sendData = {
          methodType: 5,
          bankCardId: this.curBankCard.id,
          drawingAmount: this.feeForm.fee,
      ...
      const now = dayjs(currentTime).format('YYYY-MM-DD HH:mm:ss')

      try {
        const res = await this.$http.withdrawal(sendData);
        this.withdrawalLoading = false;
        if (this.tabActive === "paybus" && res.data.code === 0) {
          this.showDialog = true;
          this.paybusData = {
            dingdan: res.data.withdrawInfo.cardNo,
            tkfs: "paybus提款",
            tkkh: this.curBankCard.cardNo,
            tkje: this.feeForm.fee,
            tksj: now,
          };
        }
    ...
    feeFormSubmit () {
      this.$refs["feeForm"].validate((valid) => {
        if (valid) {
          if (
            !this.bankcardData[this.tabActive === 'paybus' ? 'rmb' : this.tabActive] ||
            !this.bankcardData[this.tabActive === 'paybus' ? 'rmb' : this.tabActive].length
          ) {
            let msg = this.tabActive === 'otherpay' ? '请先添加EBPAY' : (this.tabActive === 'rmb' || this.tabActive === 'paybus') ? '请先添加银行卡！' : this.tabActive === 'crypto' ? '请先添加虚拟货币钱包！' : this.tabActive === 'ecny'? '请先添加数字人民币!' :'请先添加支付宝！'
  ...
  computed: {
    ...
    minAmount () {
      ...
      return this.tabActive === 'otherpay' ? ebpayMin : (this.tabActive === 'rmb' || this.tabActive === 'paybus' || this.tabActive === 'ecny') ? lowQuota : this.tabActive === 'crypto' ? lowUsdt : lowAlipayQuota
    },
    maxAmount () {
      const { topQuota = '', topUsdt = '', topAlipayQuota = '', ebpayMax = '' } = this.withdrawalCond || {}
      return this.tabActive === 'otherpay' ? ebpayMax : (this.tabActive === 'rmb' || this.tabActive === 'paybus' || this.tabActive === 'ecny') ? topQuota : this.tabActive === 'crypto' ? topUsdt : topAlipayQuota
    },
    feePlaceholder () {
