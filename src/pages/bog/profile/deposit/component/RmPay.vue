<template>
  <div class="cr-pay-container rm">
    <div v-if="!showLoading && !noData">
      ...
      <div class="user-name-container bog el-form" v-if="curPayChannel.paymentType == 13 && curPayChannel.depositName && curPayChannel.alipayFlg !== 2">
        <span>存款姓名：</span>
      ...
      <div class="select-pay-form">
        ...
        <div class="select-box rm-box">
          <div class="select-card-container">
            <div :class="{'select-item rm-select-item':true, on:recommendMoneyIndex === index, 'last-line': (index + 1) % 5 === 0}" v-for="(item,index) in recommendList" :key="index" @click="changeRecommendMoneyIndex(index,item)">{{item.label}}元</div>
          </div>
          ...
          <div class="tip-text" v-html="caclTipsTxt()">
          </div>
        </div>
      </div>
    ...
    <global-dialog class="deposit-dialog" :showDialog.sync="showDialogEcny" :title="dialogTitle" :hasSelfFooter="hasSelfFooter" :cancelText="cancelText" :confirmText="confirmText" @confirmHandle="confirmHandle" width="600px">
      <EcnyPaymentDialog :bankDialogData="ecnyBankDialogData" :curRmbChannel="renderList[payListIndex]" />
    </global-dialog>
...
import EcnyPaymentDialog from './EcnyPaymentDialog.vue'
...
export default {
  ...
  data () {
    return {
      ...
      money: null,
      renderList: [
        { label: "极速存款", children: [], imageUrl: 'new_paybus' },
        ...
        { label: "数字人民币", children: [], imageUrl: 'new_ecny' },
      ],
      bankList: [],
      noticeTextData,
      feeForm: {
  ...
  methods: {
    caclTipsTxt () {
      if (this.renderList[this.payListIndex].label ==="极速存款" || this.renderList[this.payListIndex].label ==="数字人民币") {
        return this.noticeTextData[this.renderList[this.payListIndex].label].join('<br/>');
      }
      if (this.renderList[this.payListIndex].label ==="本地银行") {
        return this.noticeTextData[this.renderList[this.payListIndex].label]
      }
      return this.noticeTextData[this.renderList[this.payListIndex].label]+'请按照提交后页面实际显示金额充值，否则无法自动到账！'
    },
    //- 充值类型切换
    ...
    getPzpayPictureUrl (response) {
      ...
      //- 本地银行
      res.bankTransferList && this.renderList[1].children.push(...res.bankTransferList);
      //- 支付宝
      res.alipayList && res.alipayList.length && this.renderList[2].children.push(...res.alipayList);
      //- 微信添加内容
      res.weChatList && res.weChatList.length && this.renderList[3].children.push(...res.weChatList);
      //-  网银支付
      res.wyList && res.wyList.length && this.renderList[4].children.push(...res.wyList);
      ...
      if (res.qrCodeList && res.qrCodeList.length) {
        ...
        this.renderList[5].children.push(...res.qrCodeList);
      }
      //- QQ支付
      res.qqList && res.qqList.length && this.renderList[6].children.push(...res.qqList);
      //- paybus
      res.jscPayList && res.jscPayList.length && this.renderList[0].children.push(...res.jscPayList)
      //- 数字人民币（数字人民币通道有两个数据来源，另一个通过接口/fastBankList获取）
      if (res.ecnyPayList && res.ecnyPayList.length) {
        res.ecnyPayList.forEach(item => {
          item.bankLog = require("../../img/deposit/ecny.png");
        })
        this.renderList[7].children.push(...res.ecnyPayList);
      }
      if (payTypeList && payTypeList.length) {
        this.renderList = this.renderList.map(item => {
          const pickRes = payTypeList.filter((itemPayType) => {
            if (itemPayType.name === '网银快捷') {
              itemPayType.name = '网银支付'
            }
            if (itemPayType.name === 'Paybus') {
              itemPayType.name = '极速存款'
            }
            return itemPayType.name === item.label
          })
        ...
        fastBanks.forEach((item) => {
          if (item.alipayFlg == 0) {
            ...
            item.bankCards.forEach((items) => {
              ...
              items.isHot = item.isHot
              items.isRecommend = item.isRecommend
              items.bankLog = require("../../img/deposit/ecny.png");
              ecnyList.push(items);
            });
          }
        });
      }
      this.renderList[2].children.unshift(...aliList);
      this.bankList.forEach(item => item.alipayFlg = 1);
      this.renderList[1].children.unshift(...this.bankList);
      // this.renderList[7].children = ecnyList;
      this.renderList[7].children.push(...ecnyList);
    ...
    async _rmPayDeposit () {
      ...
      //- 需要输入用户名
      if (this.curPayChannel.paymentType === 13 && this.curPayChannel.depositName && this.curPayChannel.alipayFlg !== 2  && !this.accountName) return this.messageTip({ message: "存款人姓名不能为空" });
    ...
    openDialogEcny ({ componentName, dialogTitle, confirmText, hasSelfFooter, cancelText }) {
      this.dialogTitle = dialogTitle;
      this.confirmText = confirmText;
      this.cancelText = cancelText;
      this.componentName = componentName
      this.showDialogEcny = true;
      this.hasSelfFooter = hasSelfFooter;
    },
    //- 立即存款
    async _onlinePayDeposit () {
      ...
      if (this.curPayChannel.payType === 'qr') {
        ...
        } else if (this.curPayChannel.alipayFlg === 2) {
          try {
            let depositPostscript = ''
            const res = await this.$http.getPayUrlBank({
              terminal: 0,
              userName: this.userInfo.realName,
              // userName: this.userInfo.realName,
            ...
            if (res.data && res.data.code === 0) {
              depositPostscript = res.data.info.depositPostscript
            }
            ...
            this.buttonLoading = false;
            this.ecnyBankDialogData = {
              '收款姓名：': this.curPayChannel.realName,
              '数字人民币编号：': this.curPayChannel.bankAccount,
              '充值金额：': this.feeForm.fee,
              // '附&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;言：': depositPostscript,
            };
            //- 大额支付专用
            this.openDialogEcny({ componentName: "EcnyPaymentDialog", dialogTitle: '数字人民币充值', cancelText: "已完成充值", confirmText: "充值问题需要协助" })
          } catch (error) {
            console.log(error)
            this.buttonLoading = false;
            this.$refs["feeForm"].resetFields();
